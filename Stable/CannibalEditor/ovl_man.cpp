...
/*
	if ((in_MouseX > px-16) && (in_MouseX < px+dx+16) && (in_MouseY > py-16) && (in_MouseY < py+dy+16))
	{
		if ((ovl->children) && (!(ovl->flags & OVLF_MINIMIZED)))
		{
			...
			{
				if (child->flags & (OVLF_NODRAW|OVLF_ALWAYSTOP))
					continue;
				mx = in_MouseX; my = in_MouseY; OVL_MousePosWindowRelative(child, &mx, &my);
				if ((mx > -16) && (mx < child->dim.x+16)
				 && (my > -16) && (my < child->dim.y+16))
				{
					drchild = child;
					break;
				}
			}
			for (child = ovl->children->next; (child != ovl->children) && (!drchild); child = child->next)
			{
				if (child->flags & OVLF_NODRAW)
					continue;
...
//	redraw = 0;
//	if (ovl->flags & (OVLF_REDRAW|OVLF_REDRAWSWAP))
		redraw = 1;
	...
	if ((redraw) && (!(ovl->flags & OVLF_NODRAW)))
	{
		...
		{			
			...
			p[0].Set(px, py, 0);
			p[1].Set(px+dx-1, py+dy-1, 0);
			c[0].Set(128, 128, 128); c[1].Set(48, 48, 48); vid.DrawLineBox(&p[0], &p[1], &c[0], &c[1], false);
			p[0].x += 1; p[0].y += 1; p[1].x -= 1; p[1].y -= 1;
			c[0].Set(255, 255, 255); c[1].Set(255, 255, 255); vid.DrawLineBox(&p[0], &p[1], &c[0], &c[1], false);
			p[0].x += 1; p[0].y += 1; p[1].x -= 1; p[1].y -= 1;
			c[0].Set(48, 48, 48); c[1].Set(128, 128, 128); vid.DrawLineBox(&p[0], &p[1], &c[0], &c[1], false);
		...
		{			
			vid.ColorMode(VCM_GOURAUD);
			p[0].Set(px-1, py+10, 0);
			p[1].Set(px+dx+1, py+10, 0);			
			c[0].Set(192, 192, 192); c[1].Set(192, 192, 192); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false); p[0].y += 1; p[1].y += 1;
			c[0].Set(255, 255, 255); c[1].Set(255, 255, 255); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false); p[0].y += 1; p[1].y += 1;
			c[0].Set(64, 64, 64); c[1].Set(64, 64, 64); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false);
			...
			if (OVL_ClipToBoxLimits(px, py, px+dx, py+9, clip))
			{
				...
				if ((!ovl->parent) || (ovl == ovl->parent->children->next))
				{
					...
					c[3].Set(32, 0, 96);
					vid.ColorMode(VCM_GOURAUD);
					vid.DrawPolygon(4, p, c, NULL, NULL, false);
				}
				...
				if (!(ovl->flags & OVLF_NOTITLEMINMAX))
					len += 20;
				int tsize = (dx-len) / strlen(ovl->name);
				if (tsize > 8)
					tsize = 8;
				...
				if (!(ovl->flags & OVLF_NOTITLEDESTROY))
				{
					p[0].Set(px+dx-9, py, 0);
					p[1].Set(px+dx-1, py+8, 0);
					c[0].Set(255, 255, 255); c[1].Set(255, 255, 255); vid.DrawLineBox(&p[0], &p[1], &c[0], &c[1], false);
				...
				if (!(ovl->flags & OVLF_NOTITLEMINMAX))
				{
					if (ovl->flags & OVLF_MINIMIZED)
					{
						p[0].Set(px+dx-19, py, 0);
						p[1].Set(px+dx-10, py+8, 0);
						c[0].Set(255, 255, 255); c[1].Set(255, 255, 255); vid.DrawLineBox(&p[0], &p[1], &c[0], &c[1], false);
						vid.DrawString(px+dx-19, py+1, 8, 8, "+", true, 128, 128, 128);
						p[0].Set(px+dx-29, py, 0);
						p[1].Set(px+dx-20, py+8, 0);
						c[0].Set(255, 255, 255); c[1].Set(255, 255, 255); vid.DrawLineBox(&p[0], &p[1], &c[0], &c[1], false);
	...
	if ((redraw) && (!(ovl->flags & (OVLF_NODRAW|OVLF_MINIMIZED))))
	{
		if (ovl->flags & OVLF_HSCROLL)
		{
			...
			if (OVL_ClipToBoxLimits(px, py+dy-11, px+dx, py+dy, clip))
			{
				...
				vid.ColorMode(VCM_GOURAUD);
				p[0].Set(px-1, py+dy-10, 0);
				p[1].Set(px+dx+1, py+dy-10, 0);			
				c[0].Set(192, 192, 192); c[1].Set(192, 192, 192); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false); p[0].y += 1; p[1].y += 1;
				c[0].Set(255, 255, 255); c[1].Set(255, 255, 255); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false); p[0].y += 1; p[1].y += 1;
				c[0].Set(64, 64, 64); c[1].Set(64, 64, 64); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false);
				p[0].y += 4; p[1].y += 4;
				c[0].Set(128, 128, 128); c[1].Set(128, 128, 128);
				vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false);
				lx = ovl->vmin.x;
				hx = ovl->vmax.x;
				if (ovl->vpos.x < lx)
					lx = ovl->vpos.x;
				...
				ss = px + (ovl->vpos.x - lx) * frac;
				se = ss + dx * frac;
				p[0].Set(ss, py+dy-7, 0);
				p[1].Set(se+1, py+dy-7, 0);
				p[2].Set(se+1, py+dy-1, 0);
				p[3].Set(ss, py+dy-1, 0);
		...
		if (ovl->flags & OVLF_VSCROLL)
		{
			...
			if (OVL_ClipToBoxLimits(px+dx-11, py, px+dx, py+dy, clip))
			{
				...
				vid.ColorMode(VCM_GOURAUD);
				p[0].Set(px+dx-10, py-1, 0);
				p[1].Set(px+dx-10, py+dy+1, 0);
				c[0].Set(192, 192, 192); c[1].Set(192, 192, 192); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false); p[0].x += 1; p[1].x += 1;
				c[0].Set(255, 255, 255); c[1].Set(255, 255, 255); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false); p[0].x += 1; p[1].x += 1;
				c[0].Set(64, 64, 64); c[1].Set(64, 64, 64); vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false);
				p[0].x += 4; p[1].x += 4;
				c[0].Set(128, 128, 128); c[1].Set(128, 128, 128);
				vid.DrawLine(&p[0], &p[1], &c[0], &c[1], false);
				...
				ss = py + (ovl->vpos.y - ly) * frac;
				se = ss + dy * frac;
				p[0].Set(px+dx-7, ss, 0);
				p[1].Set(px+dx-1, ss, 0);
				p[2].Set(px+dx-1, se+1, 0);
				p[3].Set(px+dx-7, se+1, 0);
				vid.ColorMode(VCM_FLAT);
				vid.FlatColor(192, 192, 192);
				vid.DrawPolygon(4, p, NULL, NULL, NULL, false);
		...
		if (!(ovl->flags & OVLF_NOERASECOLOR & OVLF_NOERASEDEPTH))
		{
			if (OVL_ClipToBoxLimits(px, py, px+dx, py+dy, clip))
			{
				if (ovl->flags & OVLF_NOERASECOLOR)
					vid.ColorActive(false);
				if (ovl->flags & OVLF_NOERASEDEPTH)
					vid.DepthActive(false);
				vid.ClearScreen();
				if (ovl->flags & OVLF_NOERASECOLOR)
					vid.ColorActive(true);
				if (ovl->flags & OVLF_NOERASEDEPTH)
	...
	{ // draw normal children first
		...
		if (kidclip.ex > (I32)vid.resolution->width) kidclip.ex = vid.resolution->width;
		if (kidclip.ey > (I32)vid.resolution->height) kidclip.ey = vid.resolution->height;
...
static boolean OVL_ResizeLimits(overlay_t *ovl)
{
...
static boolean OVL_RecursiveInputEvent(overlay_t *ovl, inputevent_t *event)
{
	int px, py, dx, dy;
	...
	vector_t olddim;
	char *bindcmd;
	static boolean ovlDragDropMode=0;
	static overlay_t *ovlDragDropOverlay=NULL;
	char *bodycursor;

	px = event->mouseX - ovl->pos.x;
	py = event->mouseY - ovl->pos.y;
	dx = dy = 0;
	if (!(ovl->flags & OVLF_NOBORDER))
	{			
		dx += 3; dy += 3;
	...
	if ((ovl->parent) && (!(ovl->flags & OVLF_VIEWABSOLUTE)))
	{
		px += ovl->parent->vpos.x;
		py += ovl->parent->vpos.y;
		dx -= ovl->parent->vpos.x;
		dy -= ovl->parent->vpos.y;
	}
		...
		if (event->eventType == INEV_MOUSEMOVE)
		{
			...
			if (!ovl_lockOverlay)
			{
				...
				{ // check topmost-flagged children first
					...
					child->flags |= OVLF_TOUCH;
					outevent = *event;
					outevent.mouseX -= ovl->pos.x + dx;
					outevent.mouseY -= ovl->pos.y + dy;
...
static boolean OVL_RecursiveCheckTagDestroy(overlay_t *ovl)
{
	overlay_t *child;
...
void OVL_Frame()
{
	...
	if (!ovl_Windows)
		return;
	clip.sx = clip.sy = 0; clip.ex = vid.resolution->width; clip.ey = vid.resolution->height;
	...
	if (ovl) // if this is non-null, it must be maximized
	{
		...
		ovl->pos.Set(0, 0, 0);
		ovl->dim.Set(vid.resolution->width, vid.resolution->height, 0);
		ovl->OnResize();
		OVL_RecursiveDrawOverlay(ovl, ovl->pos.x, ovl->pos.y, ovl->dim.x, ovl->dim.y, &clip);
		ovl->pos = oldpos;
		ovl->dim = olddim;
		ovl->flags &= ~OVLF_VIEWABSOLUTE;
		ovl->flags |= oldflags;
	...
	{
		OVL_RecursiveDrawOverlay(ovl_Windows, 0, 0, vid.resolution->width, vid.resolution->height, &clip);
	}
...
void OVL_InputEvent(inputevent_t *event)
{
	...
	if ((outevent.eventType == INEV_MOUSEMOVE) || (outevent.key >= KEY_FIRSTMOUSEKEY))
	{
		...
		{
			outevent.mouseX -= ovl_Windows->pos.x;
			outevent.mouseY -= ovl_Windows->pos.y;
...
void OVL_CreateRootWindow()
{
	if (!ovl_Windows)
	{
		ovl_Windows = OVL_CreateOverlay("OWorkspace", "Cannibal", (overlay_t *)-1, 0, 0, vid.resolution->width, vid.resolution->height,
...
void OVL_SetRedraw(overlay_t *ovl, boolean extreme)
{
/*	
	overlay_t *child;
...
boolean OVL_ClipToBoxLimits(int sx, int sy, int ex, int ey, ovlClipBox_t *clip)
{
	if (sx < clip->sx)
		sx = clip->sx;
	if (sy < clip->sy)
	...
	if (ey > clip->ey)
		ey = clip->ey;
	if ((ex <= sx) || (ey <= sy) || (ex <= 0) || (ey <= 0) || (sx >= (I32)vid.resolution->width) || (sy >= (I32)vid.resolution->height))
		return(0);
	...
	if (ovl->parent)
	{
		if (!(ovl->flags & OVLF_MAXIMIZED))
		{
			...
			*my -= ovl->pos.y;
            ...
            {
			    *mx += ovl->parent->vpos.x;
			    *my += ovl->parent->vpos.y;
            }
		}
	}
	else
...
boolean OVL_SendMsg(overlay_t *ovl, char *msgname, int numparms, ... )
{
	ovlmsg_t fullmsg;
	va_list args;
	int i;
...
void OVL_SendMsgAll(overlay_t *ovl, char *msgname, int numparms, ... )
{
	ovlmsg_t fullmsg;
	va_list args;
	int i;
...
boolean OVL_SendPressCommand(overlay_t *ovl, char *text, ... )
{
...
boolean OVL_SendDragCommand(overlay_t *ovl, char *text, ... )
{
	static char tbuffer[1024];
	va_list args;
	int res;
...
boolean OVL_SendReleaseCommand(overlay_t *ovl, char *text, ... )
{
	static char tbuffer[1024];
	va_list args;
	int res;
...
void OVL_SaveOverlay(overlay_t *ovl) // save overlay (and all child windows) to clipboard
{
	...
	VCR_EnlargeActionDataBuffer(4+strlen(ovl->name)+strlen(ovl->typedecl->ovlTypeName)+4+32);
...
void OVL_LoadOverlay(overlay_t *parent) // create and load overlay and its children from clipboard, to the given parent
{
	...
	if (version == 1)
	{
		...
		{
			ovl = OVL_CreateOverlay(ltype, lname, parent, lpos.x, lpos.y, ldim.x, ldim.y, lflags, false);
			ovl->vpos.x = VCR_ReadFloat();
			ovl->vpos.y = VCR_ReadFloat();
			ovl->vpos.z = 0;
			if (ovl->flags & OVLF_HASFOCUS)
...
boolean OVL_IsOverlayType(overlay_t *ovl, char *ovltype)
{
	if ((!ovl) || (!ovl->typedecl))
		return(0);
...
{
	overlay_t *ovl = OVL_CreateOverlay("OInputBox", caption, NULL, 320 - (strlen(text)*8 + 10)/2, 220, strlen(text)*8 + 10, 40,
		OVLF_ALWAYSTOP|OVLF_VIEWABSOLUTE|OVLF_NODRAGDROP|OVLF_NOMOVE|OVLF_NORESIZE|OVLF_NOTITLEMINMAX|OVLF_NOTITLEDESTROY, false);
...
overlay_t::overlay_t(overlay_t *parentwindow) // used only for dummy nodes
{
	...
	pos.Set(0, 0, 0);
	//dim.Set(vid.resolution->width, vid.resolution->height, 0);
	dim.Set(640, 480, 0);
	vpos = pos;
	vmin.Set(0, 0, 0);
	vmax = dim;
...
void overlay_t::OnCalcLogicalDim(int dx, int dy)
{
	...
	{	
		...
		if ((child->pos.x + child->dim.x) > maxx)
			maxx = child->pos.x + child->dim.x;
		if (!(child->flags & OVLF_MINIMIZED))
		{
			if ((child->pos.y + child->dim.y) > maxy)
				maxy = child->pos.y + child->dim.y;
		}
		else
		{
			if ((child->pos.y + 12+3+3) > maxy)
				maxy = child->pos.y + 12+3+3;
		}
	}
	vmin.Set(minx, miny, 0);
	vmax.Set(maxx, maxy, 0);
}
...
boolean overlay_t::OnMouseMove(inputevent_t *event)
{
	return(0);
}

boolean overlay_t::OnPress(inputevent_t *event)
{
	return(0);
}

boolean overlay_t::OnDrag(inputevent_t *event)
{
	return(0);
}

boolean overlay_t::OnRelease(inputevent_t *event)
{
	return(0);
}

boolean overlay_t::OnPressCommand(int argNum, char **argList)
{
	return(0);
}

boolean overlay_t::OnDragCommand(int argNum, char **argList)
{
	return(0);
}

boolean overlay_t::OnReleaseCommand(int argNum, char **argList)
{
	return(0);
}

boolean overlay_t::OnMessage(ovlmsg_t *msg)
{
	return(0);
}

boolean overlay_t::OnDragDrop(overlay_t *dropovl)
{
	return(0);
}
