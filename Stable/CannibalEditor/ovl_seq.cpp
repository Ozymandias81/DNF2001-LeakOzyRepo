...
void OSequence::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	p[0].Set(sx, sy-adjy+12, 0);
	p[1].Set(sx+dx, sy-adjy+12, 0);
	vid.DrawLine(&p[0], &p[1], NULL, NULL, false);
	p[0].Set(sx+192+1, sy-adjy, 0);
	p[1].Set(sx+192+1, sy-adjy+12, 0);
	vid.DrawLine(&p[0], &p[1], NULL, NULL, false);
...
//		vid.DrawString(sx+256+2, sy+i*12+2-adjy, 8, 8, tbuffer, true, 128, 128, 128);
		/*
		if (item->trigger)
			vid.DrawString(sx+256+32+2, sy+i*12+2-adjy, 8, 8, item->trigger, true, 128, 128, 128);
		*/
		vid.ColorMode(VCM_FLAT);
		vid.FlatColor(255,255,255);
		p[0].Set(sx, sy+i*12-adjy+12, 0);
		p[1].Set(sx+192+1, sy+i*12-adjy+12, 0);
		vid.DrawLine(&p[0], &p[1], NULL, NULL, false);
		p[0].Set(sx+192+1, sy+i*12-adjy, 0);
		p[1].Set(sx+192+1, sy+i*12-adjy+12, 0);
		vid.DrawLine(&p[0], &p[1], NULL, NULL, false);
//		p[0].Set(sx+256+32+1, sy+i*12-adjy, 0);
//		p[1].Set(sx+256+32+1, sy+i*12-adjy+12, 0);
//		vid.DrawLine(&p[0], &p[1], NULL, NULL, false);
	}
	for (trig=seq->triggers.prev;trig!=&seq->triggers;trig=trig->prev)
	{
		if (trig->triggerBinData)
			continue;
		int len;
		i = trig->trigTimeFrac*seq->numItems*12+12-adjy + 0.5;
		len = strlen(trig->trigger);
		if (trig->flags & SEQITEMF_SELECTED)
		{
			p[0].Set(sx+228,sy+i-4,0);
			p[2].Set(sx+228+len*6+4,sy+i+4,0);
		}
		else
		{
			p[0].Set(sx+200,sy+i-4,0);
			p[2].Set(sx+200+len*6+4,sy+i+4,0);
		}
		p[1].Set(p[2].x, p[0].y, 0);
		p[3].Set(p[0].x, p[2].y, 0);
		vid.ColorMode(VCM_FLAT);
		...
		vid.DrawLineBox(&p[0], &p[2], NULL, NULL, false);
		p[2] = p[0];
		p[3] = p[1];
		p[3].x = p[2].x;
		p[3].y += 4;
		...
		if (trig->flags & SEQITEMF_SELECTED)
		{
			vid.DrawString(p[0].x+2, p[0].y+2, 6, 6, trig->trigger, true, 255, 255, 0);
			sprintf(tbuffer, "%1.3f", trig->trigTimeFrac);
			vid.DrawString(p[0].x-33, p[0].y-2, 6, 6, tbuffer, true, 255, 255, 255);
		}
		else
			vid.DrawString(p[0].x+2, p[0].y+2, 6, 6, trig->trigger, true, 128, 128, 128);
	}
}

static int seq_trigDragY=-1; // can remain static since due to mouselock only one window could use at a time

boolean OSequence::OnPress(inputevent_t *event)
{
	int adjy;
		...
		//			item->flags |= SEQITEMF_TIMEMODE;
			...
			else
			{
				for (trig=seq->triggers.next;trig!=&seq->triggers;trig=trig->next)
				{
					...
					{
						p[0].Set(228,i-4,0);
						p[1].Set(228+len*6+4,i+4,0);
					}
					else
					{
						p[0].Set(200,i-4,0);
						p[1].Set(200+len*6+4,i+4,0);
					}
	...
	case KEY_INS:
		trig = seq->AddTrigger();
		if (selItem)
			trig->trigTimeFrac = ((float)selItemIndex / (float)seq->numItems) + (0.5 / (float)seq->numItems);
		return(1);
		break;
	case KEY_DEL:
		if (selItem)
...
boolean OSequence::OnDrag(inputevent_t *event)
{
	seqTrigger_t *trig;
	...
	trig->trigTimeFrac += (float)diffy / (12.0*seq->numItems);
...
boolean OSequence::OnRelease(inputevent_t *event)
{
...
boolean OSequence::OnPressCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("seqrate")
	{
	...
	OVLCMD("triggername")
	{
		...
		if (trig->trigger)
			FREE(trig->trigger);
		trig->trigger = ALLOC(char, strlen(argList[2])+1);
...
//		char tbuffer[32];
		...
		if (argNum < 3)
		{
//			sprintf(tbuffer, "itemtrigger %d", k);
//			if (item->trigger)
//				OVL_InputBox("Trigger", "Enter a trigger string for this frame", this, tbuffer, item->trigger);
//			else
//				OVL_InputBox("Trigger", "Enter a trigger string for this frame", this, tbuffer, "");
			return(1);
		}
//		if (item->trigger)
//			FREE(item->trigger);
//		item->trigger = ALLOC(char, strlen(argList[2])+1);
//		strcpy(item->trigger, argList[2]);
		return(1);
	}
	OVLCMD("itemmoveup")
	{
...
/*
boolean OSequence::OnDragCommand(int argNum, char **argList)
{
}
*/

/*
boolean OSequence::OnReleaseCommand(int argNum, char **argList)
{
}
*/

/*
boolean OSequence::OnMessage(ovlmsg_t *msg)
{
}
*/
