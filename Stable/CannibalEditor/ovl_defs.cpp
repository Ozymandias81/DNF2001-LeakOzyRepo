...
//    Private Code
//----------------------------------------------------------------------------
static unsigned long OVL_OverlayFlagForName(char *name)
{
	int i, flag;
...
boolean OWindow::OnMessage(ovlmsg_t *msg)
{
	OVLMSGSTART
...
boolean OWindow::OnPressCommand(int argNum, char **argList)
{	
	OVLCMDSTART
	OVLCMD("minimize")
	{
...
boolean OWindowScrollable::OnPressCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("mscroll")
	{
...
boolean OWindowScrollable::OnDragCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("mscroll")
	{
...
boolean OWindowScrollable::OnReleaseCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("mscroll")
	{
...
boolean OConsole::OnMessage(ovlmsg_t *msg)
{
	OVLMSGSTART
...
boolean OConsole::OnPressCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("toggleconsole")
	{
...
void OConsole::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	p[0].Set(sx, ey-10, 0);
	p[1].Set(ex, ey-10, 0);
	c[0].Set(255, 255, 255); c[1].Set(255, 255, 255);
	...
	cmdOverflow = strlen(tempbuffer) - (cmdDisplayWidthChars-2);
	if (cmdOverflow < 0)
		cmdOverflow = 0;
	if (clicker)
		strcat(tempbuffer, "_");
...
boolean OConsole::OnPress(inputevent_t *event)
{
	if (!target)
	{
		flags |= OVLF_TAGDESTROY;
	...
	case KEY_DOWNARROW:
		...
		break;
	case KEY_BACKSPACE:
		if (strlen(CON->cmdLine) > 0)
			CON->cmdLine[strlen(CON->cmdLine)-1] = 0;
	...
	case KEY_LALT:
		...
		break;
	default:		
		if ((vid.charDrawable[key]) // is it drawable?
			&& (!(event->flags & (KF_CONTROL|KF_ALT))))
		{
			if (event->flags & KF_SHIFT)
				key = in_Shifted[key];
			tempstr[0] = (byte)key; tempstr[1] = 0;
			if ((strlen(CON->cmdLine)+2) < CON_MAXLINELEN)
