...
typedef struct
{
	unsigned long marker; // "ASCF" for Apogee Software Chunk Format
	unsigned long typeMarker; // File-type specific marker
    short ascfVersion; // ASCFVERSION
	short typeVersion; // File-type specific version
	unsigned long fileSize; // total size of file
	unsigned long dirOfs; // chunk directory offset
    unsigned long dirEntries; // chunk directory entry count (directory size = dirEntries*sizeof(ascfentry_t))
    unsigned long user1; // additional user data space
    unsigned long user2; // additional user data space
} ascfheader_t;

// ASCF Standard Chunk Entry
typedef struct
{
	unsigned long chunkLabel; // four-character label, see list below
	unsigned long chunkOfs; // chunk data offset
	unsigned long chunkLen; // chunk data length in bytes
    unsigned char chunkVersion; // chunk-specific version number
...
typedef struct
{
	byte groupNum; // group number for byte decompression
	byte v[3];
	byte normal[3]; // s.7, s = sign bit, .7 = fabs(component*127);
    byte mountIndex; // mount point for basis
...
{
	mdxframeinfo_t frameInfo; // geometry information for frame
	long triInfoOfs; // byte offset into info block of triInfo
