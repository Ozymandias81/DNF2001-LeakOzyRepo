...
CONVAR(vector_t, skin_curColor, vector_t(0.0f, 0.0f, 0.0f), 0, NULL);

CONVAR(U32, skin_hsvMode, 1, 0, NULL);
//----------------------------------------------------------------------------
//    Public Data
//----------------------------------------------------------------------------
U32 _draw_front=0;
...
//    Private Code
//----------------------------------------------------------------------------
#define RETURN_HSV(h,s,v) {HSV.x=(h)*255.0f/6.0f; HSV.y=(s)*255.0f; HSV.z=(v)*255.0f; return HSV;}
#define RETURN_RGB(r,g,b) {RGB.x=(r)*255.0f; RGB.y=(g)*255.0f; RGB.z=(b)*255.0f; return RGB;}
...
vector_t RGB2HSV(vector_t RGB)
{
    ...
    float R=RGB.x/255.0f, G=RGB.y/255.0f, B=RGB.z/255.0f, v,x,f;
    int i;
    vector_t HSV;
    x=min3(R,G,B);
    v=max3(R,G,B);
...
vector_t HSV2RGB(vector_t HSV)
{
    ...
    float h=HSV.x*6.0f/255.0f, s=HSV.y/255.0f, v=HSV.z/255.0f, m,n,f;
    int i;
    vector_t RGB;
    if(s==0.0f) RETURN_RGB(v,v,v);
    i=(int)h;
    f=h-i;
...
/*
static U32 PointInPoly(vector_t &p, plane_t &tplane, vector_t &p1, vector_t &p2, vector_t &p3)
{
	int i;
...
void OPalette::InvalidatePalColors()
{
	...
	for (i=0;i<32;i++)
	{
		for (k=0;k<32;k++)
		{
			...
			palColors[i][k].y = (float)(k<<3);
			palColors[i][k].z = (float)(i<<3);
			if (skin_hsvMode)
			{ // hsv
				palColors[i][k] = HSV2RGB(palColors[i][k]);
			}
		}
		palColors[32][i].x = (float)(i<<3);
		palColors[32][i].y = 0;
		palColors[32][i].z = 0;
		if (skin_hsvMode)
		{ // hsv
...
void OPalette::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	for (i=0;i<32;i++)
	{
		for (k=0;k<32;k++)
		{
			p[0].Seti(x+(k<<2), y+(i<<2), 0);
			p[1].Seti(x+(k<<2)+4, y+(i<<2), 0);
			p[2].Seti(x+(k<<2)+4, y+(i<<2)+4, 0);
			p[3].Seti(x+(k<<2), y+(i<<2)+4, 0);
			vid->FlatColorf(palColors[i][k].x, palColors[i][k].y, palColors[i][k].z);
			vid->DrawPolygon(4, p, NULL, NULL, NULL);
		}
		p[0].Seti(x+(i<<2), y+128, 0);
		p[1].Seti(x+(i<<2)+4, y+128, 0);
		p[2].Seti(x+(i<<2)+4, y+128+8, 0);
		p[3].Seti(x+(i<<2), y+128+8, 0);
		vid->FlatColorf(palColors[32][i].x, palColors[32][i].y, palColors[32][i].z);
		vid->DrawPolygon(4, p, NULL, NULL, NULL);
	}
	...
	p[0].Setf(x+dim.x-26,(float)(y-11), 0);
	p[2].Setf(x+dim.x-18,(float)(y-4), 0);
	p[1].Setf(p[2].x, p[0].y, 0.0f);
	p[3].Setf(p[0].x, p[2].y, 0.0f);
	vid->FlatColorf(palColor.x, palColor.y, palColor.z);
	vid->DrawPolygon(4, p, NULL, NULL, NULL);
	vid->DepthEnable(TRUE);
}

U32 OPalette::OnPress(inputevent_t *event)
{
	...
	if (event->mouseY < 128)
	{
		skin_curColor.y = (float)((event->mouseX>>2)<<3);
		skin_curColor.z = (float)((event->mouseY>>2)<<3);
	}
	else
	{
		skin_curColor.x = (float)((event->mouseX>>2)<<3);
		InvalidatePalColors();
	}
	return(1);
}
...
/*
U32 OPalette::OnDrag(inputevent_t *event)
{
}
*/

/*
U32 OPalette::OnRelease(inputevent_t *event)
{
}
*/

/*
U32 OPalette::OnPressCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OPalette::OnDragCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OPalette::OnReleaseCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OPalette::OnMessage(ovlmsg_t *msg)
{
}
*/
...
void OVL_SkinPaint(modelSkin_t *skin, int px, int py, int size, float alpha, U32 reload, U32 aa)
{
	...
	if (size > 0)
	{
		OVL_SkinPaint(skin, px, py, 0, 1.0f, reload, aa);
		OVL_SkinPaint(skin, px-1, py, 0, 0.4f, reload, aa);
		OVL_SkinPaint(skin, px+1, py, 0, 0.4f, reload, aa);
		OVL_SkinPaint(skin, px, py-1, 0, 0.4f, reload, aa);
		OVL_SkinPaint(skin, px, py+1, 0, 0.4f, reload, aa);
		if (size > 1)
		{
			OVL_SkinPaint(skin, px-1, py-1, 0, 0.25f, reload, aa);
			OVL_SkinPaint(skin, px+1, py-1, 0, 0.25f, reload, aa);
			OVL_SkinPaint(skin, px+1, py+1, 0, 0.25f, reload, aa);
			OVL_SkinPaint(skin, px-1, py+1, 0, 0.25f, reload, aa);
		}
	...
	*((int *)buf) = val;
	...
	if (aa)
	{
		r = (int)((((wv >> 10) & 31) << 3) * (1.0f-alpha) + (r * alpha));
		g = (int)((((wv >> 5) & 31) << 3) * (1.0f-alpha) + (g * alpha));
		b = (int)(((wv & 31) << 3) * (1.0f-alpha) + (b * alpha));
	}
	r >>= 3; g >>= 3; b >>= 3;
	if (skin->tex->bpp==2)
		((U16 *)skin->tex->tex_data)[py*skin->tex->width+px] = (r << 10) + (g << 5) + b;
...
void OVL_SkinLine(modelSkin_t *skin, int x1, int y1, int x2, int y2, int size, float alpha, U32 aa)
{
	...
	if (size > 0)
	{
		OVL_SkinLine(skin, x1, y1, x2, y2, 0, 1.0f, aa);
		OVL_SkinLine(skin, x1-1, y1, x2-1, y2, 0, 0.4f, aa);
		OVL_SkinLine(skin, x1, y1-1, x2, y2-1, 0, 0.4f, aa);
		OVL_SkinLine(skin, x1, y1+1, x2, y2+1, 0, 0.4f, aa);
		OVL_SkinLine(skin, x1+1, y1, x2+1, y2, 0, 0.4f, aa);
		if (size > 1)
		{
			OVL_SkinLine(skin, x1-1, y1-1, x2-1, y2-1, 0, 0.25f, aa);
			OVL_SkinLine(skin, x1-1, y1+1, x2-1, y2+1, 0, 0.25f, aa);
			OVL_SkinLine(skin, x1+1, y1-1, x2+1, y2-1, 0, 0.25f, aa);
			OVL_SkinLine(skin, x1+1, y1+1, x2+1, y2+1, 0, 0.25f, aa);
		}
...
void OSkinView::OnSave()
{
	...
	VCR_WriteByte((U8)skinantialias);
	VCR_WriteByte((U8)skinfiltered);
	VCR_WriteByte((U8)refOverride);
	VCR_WriteByte((U8)selectionMarks);
...
void OSkinView::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	p[0].Setf((float)(-((I32)skin->tex->width/2)),(float)(skin->tex->height/2), 0.1f);
	p[1].Setf((float)(skin->tex->width/2),(float)(skin->tex->height/2), 0.1f);
	p[2].Setf((float)(skin->tex->width/2),(float)(-((I32)skin->tex->height/2)), 0.1f);
	p[3].Setf((float)(-((I32)skin->tex->width/2)),(float)(-((I32)skin->tex->height/2)), 0.1f);
	vid->ColorMode(VCM_FLAT);
	vid->FlatColor(255, 255, 255);
	vid->DepthEnable(TRUE);
	camera.DrawLine(&p[0], &p[1], NULL, NULL);
	camera.DrawLine(&p[1], &p[2], NULL, NULL);
	camera.DrawLine(&p[2], &p[3], NULL, NULL);
	camera.DrawLine(&p[3], &p[0], NULL, NULL);
	...
	camera.DrawPolygon(4, p, NULL, NULL, tv);
	...
	if ((skinlineStart.x != -1) && (skinlineStart.y != -1))
	{
		...
		plane_t pln; pln.n.Seti(0, 0, 1); pln.d = 0;
		float dist = pln.IntersectionUV(lu, lv, mpoint);
		if (dist < 0.0f)
			return;
		p[0].Setf(skinlineStart.x - skin->tex->width/2, -(skinlineStart.y - skin->tex->height/2), 0.1f);
		p[1].Setf(mpoint.x, mpoint.y, 0.1f);
		p[0].x += 0.5f; p[0].y -= 0.5f;
		vid->ColorMode(VCM_FLAT);
		vid->FlatColor(255, 255, 255);
		camera.DrawLine(&p[0], &p[1], NULL, NULL);
	}
	if ((baseboxStart.x != -1) && (baseboxStart.y != -1))
	{
		...
		if (dist < 0.0f)
			return;
		xo = (int)(mpoint.x + skin->tex->width/2); yo =(int)(-mpoint.y + skin->tex->height/2);
		p[0].Setf(baseboxStart.x, baseboxStart.y, 0.1f);
		p[2].Setf(mpoint.x, mpoint.y, 0.1f);
		p[0].x += 0.5f; p[0].y -= 0.5f;
		p[1].Setf(p[2].x, p[0].y, 0.1f);
		p[3].Setf(p[0].x, p[2].y, 0.1f);
		vid->ColorMode(VCM_FLAT);
		vid->FlatColor(255, 255, 255);
		camera.DrawLine(&p[0], &p[1], NULL, NULL);
		camera.DrawLine(&p[1], &p[2], NULL, NULL);
	...
	if ((baseTris) && (wireframeActive))
	{
		...
		vid->DepthEnable(FALSE);
		U32 count_lines,count_lines2,count2_extra;
		count_lines=count_lines2=count2_extra=0;
...
count2:
		...
		vid->BeginLines((count_lines+4)*3);
		for (i=0;i<f->numTris;i++)
		{
			tri = &baseTris[i];
			ftri = &f->mdl->mesh.meshTris[i];
            ...
            if (tri->flags & BTF_HIDDEN)
                continue;
			p[0].Setf(tri->tverts[0].x - skin->tex->width/2 + 0.5f, -(tri->tverts[0].y - skin->tex->height/2) - 0.5f, 0.1f);
			p[1].Setf(tri->tverts[1].x - skin->tex->width/2 + 0.5f, -(tri->tverts[1].y - skin->tex->height/2) - 0.5f, 0.1f);
			p[2].Setf(tri->tverts[2].x - skin->tex->width/2 + 0.5f, -(tri->tverts[2].y - skin->tex->height/2) - 0.5f, 0.1f);
			camera.DrawLine(&p[0], &p[1], NULL, NULL);
			camera.DrawLine(&p[1], &p[2], NULL, NULL);
		...
		/* pad it */
		//count_lines2+=4;
		//count_lines2=(count_lines2 * 3) + (count_lines2 * 24) + count2_extra;
		//vid->BeginLines((count_lines2+4)*3);
		for (i=0;i<f->numTris;i++)
		{
			tri = &baseTris[i];
			ftri = &f->mdl->mesh.meshTris[i];
			...
			p[0].Setf(tri->tverts[0].x - width, (height - tri->tverts[0].y), 0.1f);
			p[1].Setf(tri->tverts[1].x - width, (height - tri->tverts[1].y), 0.1f);
			p[2].Setf(tri->tverts[2].x - width, (height - tri->tverts[2].y), 0.1f);
			...
			if (selectionMarks)
			{
				...
				{
					m1 = p[k]; m2 = p[(k+1)%3];
					for (frac=0.0f,n=0;n<8;n++,frac+=0.125)
					{
						m3 = (m1*(1.0f-frac)+m2*frac);
						m4 = ((c-m3)*0.15f)+m3;
						camera.DrawLine(&m3, &m4, NULL, NULL);
					}
				}
			}
			if (tri->flags & (BTF_VM0|BTF_VM1|BTF_VM2))
			{
				...
				if (tri->flags & BTF_VM0)
					camera.DrawLine(&p[0], &c, NULL, NULL);
				if (tri->flags & BTF_VM1)
					camera.DrawLine(&p[1], &c, NULL, NULL);
				if (tri->flags & BTF_VM2)
					camera.DrawLine(&p[2], &c, NULL, NULL);
				vid->FlatColor(255, 255, 255);
			}
		}
		//vid->EndLines();
...
/*
U32 OSkinView::OnPress(inputevent_t *event)
{
}
*/

/*
U32 OSkinView::OnDrag(inputevent_t *event)
{
}
*/

/*
U32 OSkinView::OnRelease(inputevent_t *event)
{
}
*/

U32 OSkinView::OnPressCommand(int argNum, CC8 **argList)
{
	OVLCMDSTART

	OVLCMD("zoomin") { if (camera.position.z > 6.0) camera.MoveForward(skin_zoomSpeed*mesh_app.get_frame_delta()); return(1); }
	OVLCMD("zoomout") { if (camera.position.z < 1000.0) camera.MoveForward(-skin_zoomSpeed*mesh_app.get_frame_delta()); return(1); }
	OVLCMD("panup") { if (camera.position.y < skin->tex->height/2) camera.MoveUp(skin_panSpeed*mesh_app.get_frame_delta()); return(1); }
	OVLCMD("pandown") { if (camera.position.y > -((I32)skin->tex->height/2)) camera.MoveUp(-skin_panSpeed*mesh_app.get_frame_delta()); return(1); }
	OVLCMD("panright") { if (camera.position.x < skin->tex->width/2) camera.MoveRight(skin_panSpeed*mesh_app.get_frame_delta()); return(1); }
	OVLCMD("panleft") { if (camera.position.x > -((I32)skin->tex->width/2)) camera.MoveRight(-skin_panSpeed*mesh_app.get_frame_delta()); return(1); }
    ...
    OVLCMD("skinpaint")
	{
		...
		*/
		...
		OVL_SkinPaint(skin, xo, yo, skinbrushsize, 1.0f, false, skinantialias);
		vid->TexReload(skin->tex);
		return(1);
	}
	OVLCMD("skinline")
	...
	OVLCMD("baseselect")
	{
		...
		if (in_keyFlags & KF_SHIFT)
		{
			...
			if (dist < 0.0f)
				return(1);
			xo = (int)mpoint.x; yo = (int)mpoint.y;
			baseboxStart.Seti(xo, yo, 0);
			OVL_LockInput(this);
		}
		else
		if (!(in_keyFlags & KF_ALT))
		{
			...
			{			
				...
				if (ftri->flags & TF_HIDDEN)
					continue;
				p[0].Setf(tri->tverts[0].x - skin->tex->width/2 + 0.5f, -(tri->tverts[0].y - skin->tex->height/2) - 0.5f, 0.0f);
				p[1].Setf(tri->tverts[1].x - skin->tex->width/2 + 0.5f, -(tri->tverts[1].y - skin->tex->height/2) - 0.5f, 0.0f);
				p[2].Setf(tri->tverts[2].x - skin->tex->width/2 + 0.5f, -(tri->tverts[2].y - skin->tex->height/2) - 0.5f, 0.0f);
	...
	OVLCMD("baserotate")
	{
		...
		if (dist < 0.0f)
			return(1);
		bftransStart.Seti(((int)(mpoint.x + skin->tex->width/2)), ((int)(-(mpoint.y - skin->tex->height/2))), 0);
		bfmode = 1;
		VCR_Record(VCRA_UNDO, "$basemanip", Undo_BaseManip, f->numTris*36 + 16, NULL);
		VCR_WriteBulk(&this->parent, 4);
		VCR_WriteBulk(&f, 4);
	...
	OVLCMD("skinnext")
	{
		...
		{
			if (skin->tex->width >= skin->tex->height)
				camera.SetPositioni(0, 0, skin->tex->width);
			else
				camera.SetPositioni(0, 0, skin->tex->height);
...
U32 OSkinView::OnDragCommand(int argNum, CC8 **argList)
{
	OVLCMDSTART
	OVLCMD("pan")
    {
		...
		if (axislockmode != 1)
		{
			if ((oldMouseY-my) >= 0)
			{
				if (camera.position.y < skin->tex->height/2)
					camera.MoveUp((float)(oldMouseY-my));
			}
			else
			{
	...
	OVLCMD("basemove")
	{
		...
		{			
			...
			{
				f->flags |= MRF_MODIFIED;
				if (axislockmode!=2) tri->tverts[0].x += mpoint.x - bftransStart.x;// tri->tverts[0][0] = (int)(tri->tverts[0][0] + 0.5f);
				if (axislockmode!=1) tri->tverts[0].y -= mpoint.y - bftransStart.y;// tri->tverts[0][1] = (int)(tri->tverts[0][1] + 0.5f);
			}
			if ((!vmode) || (tri->flags & BTF_VM1))
			{
				f->flags |= MRF_MODIFIED;
				if (axislockmode!=2) tri->tverts[1].x += mpoint.x - bftransStart.x;// tri->tverts[1][0] = (int)(tri->tverts[1][0] + 0.5f);
				if (axislockmode!=1) tri->tverts[1].y -= mpoint.y - bftransStart.y;// tri->tverts[1][1] = (int)(tri->tverts[1][1] + 0.5f);
			}
			if ((!vmode) || (tri->flags & BTF_VM2))
			{
				f->flags |= MRF_MODIFIED;
				if (axislockmode!=2) tri->tverts[2].x += mpoint.x - bftransStart.x;// tri->tverts[2][0] = (int)(tri->tverts[2][0] + 0.5f);
				if (axislockmode!=1) tri->tverts[2].y -= mpoint.y - bftransStart.y;// tri->tverts[2][1] = (int)(tri->tverts[2][1] + 0.5f);
			}
		}
		bftransStart = mpoint;
		return(1);
	...
	OVLCMD("baserotate")
	{
		...
		if (dist < 0.0f)
			return(1);
		matrix_t mxform = MatRotation(AXIS_Z, (mx - oldMouseX)*PI_f/128.0f);
		for (i=0;i<f->numTris;i++)
		{			
			tri = &baseTris[i];
			ftri = &f->mdl->mesh.meshTris[i];
	...
	OVLCMD("basescale")
	{
		...
		if (dist < 0.0f)
			return(1);
		scale = 1.0f+(my - oldMouseY)*0.05f;
		if ((my - oldMouseY) < 0)
			scale = 1.0f/(1.0f+(oldMouseY - my)*0.05f);
		for (i=0;i<f->numTris;i++)
		{			
			tri = &baseTris[i];
			ftri = &f->mdl->mesh.meshTris[i];
...
static U32 Inbox(vector_t *p, int x1, int y1, int x2, int y2)
{
	int temp;
	if (x2 < x1)
	{
...
U32 OSkinView::OnReleaseCommand(int argNum, CC8 **argList)
{
	OVLCMDSTART
	OVLCMD("skinline")
	{
		...
		VCR_Record(VCRA_UNDO, "$skinpaint", Undo_SkinPaint, 8192, NULL);
		VCR_WriteInt((unsigned long)(((OWorkspace *)this->parent)->mdx->skins));
		OVL_SkinLine(skin,(int)skinlineStart.x,(int)skinlineStart.y, xo, yo, skinbrushsize, 1.0f, skinantialias);
		vid->TexReload(skin->tex);
	...
	OVLCMD("baseselect")
	{
		...
		{			
			...
			p[1].Setf(tri->tverts[1].x - skin->tex->width/2 + 0.5f, -(tri->tverts[1].y - skin->tex->height/2) - 0.5f, 0.0f);
			p[2].Setf(tri->tverts[2].x - skin->tex->width/2 + 0.5f, -(tri->tverts[2].y - skin->tex->height/2) - 0.5f, 0.0f);
			if (Inbox(&p[0], (int)mpoint.x, (int)mpoint.y, (int)baseboxStart.x, (int)baseboxStart.y)
			 || Inbox(&p[1], (int)mpoint.x, (int)mpoint.y, (int)baseboxStart.x, (int)baseboxStart.y)
			 || Inbox(&p[2], (int)mpoint.x, (int)mpoint.y, (int)baseboxStart.x, (int)baseboxStart.y))
			{
				ftri->flags ^= TF_SELECTED;
				tri->flags &= ~(BTF_VM0|BTF_VM1|BTF_VM2);
			}
	...
	OVLCMD("basemove")
	{
		...
		{			
			...
			if ((!vmode) || (tri->flags & BTF_VM0))
			{
				tri->tverts[0].x = (float)((int)(tri->tverts[0].x + 0.5f));
				tri->tverts[0].y = (float)((int)(tri->tverts[0].y + 0.5f));
			}
			if ((!vmode) || (tri->flags & BTF_VM1))
			{
				tri->tverts[1].x = (float)((int)(tri->tverts[1].x + 0.5f));
				tri->tverts[1].y = (float)((int)(tri->tverts[1].y + 0.5f));
			}
			if ((!vmode) || (tri->flags & BTF_VM2))
			{
				tri->tverts[2].x = (float)((int)(tri->tverts[2].x + 0.5f));
				tri->tverts[2].y = (float)((int)(tri->tverts[2].y + 0.5f));
			}
		}
		return(1);
	}
...
/*
U32 OSkinView::OnMessage(ovlmsg_t *msg)
{
	OVLMSGSTART
...
/*
U32 OSkinView::OnDragDrop(overlay_t *dropovl)
{
	baseTri_t *baseTris;
