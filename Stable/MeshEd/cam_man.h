...
public:
    ...
    // internal operators
    // methods
	void Setf(float xres, float yres, float frontlim, float backlim);
	inline void Seti(U32 xres,U32 yres,U32 frontlim,U32 backlim){Setf((float)xres,(float)yres,(float)frontlim,(float)backlim);}
...
public:
	...
	inline void SetViewVolumef(float xres, float yres, float frontlim, float backlim)
	{
		frust.Setf(xres, yres, frontlim, backlim);
	}
	void SetViewVolumei(int xres, int yres, int frontlim, int backlim){SetViewVolumef((float)xres,(float)yres,(float)frontlim,(float)backlim);}
	void SetFOVf(float xfactor, float yfactor);
	void SetFOVi(I32 xfactor,I32 yfactor){SetFOVf((float)xfactor,(float)yfactor);}
	void SetPosition(vector_t &npos);
	void SetPositionf(float px, float py, float pz);
	void SetPositioni(int px,int py,int pz){SetPositionf((float)px,(float)py,(float)pz);}
	...
	void DrawLine(vector_t *p1, vector_t *p2, vector_t *c1, vector_t *c2);
	void DrawTriangle(vector_t *p /* 3 */, vector_t *c /* 3 */, float *a /* 3 */, vector_t *tv);
	void DrawTriangleFlags(U32 flags,vector_t *p,vector_t *c, float *a, vector_t *tv);
	void DrawPolygon(int numverts, vector_t *p, vector_t *c, float *a, vector_t *tv);
	void DrawPolygonFlags(U32 flags,int numverts, vector_t *p, vector_t *c, float *a, vector_t *tv);
	void DrawProjectedString(vector_t *inp, char *str, U32 filtered, int r, int g, int b);
	void DrawBox(vector_t *p1, vector_t *p2, vector_t *colors, char *str, U32 extBorder);
