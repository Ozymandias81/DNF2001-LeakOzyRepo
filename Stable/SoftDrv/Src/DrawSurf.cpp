...
//void USoftwareRenderDevice::DrawComplexSurface( FSceneNode* Frame, FSurfaceInfo& Surface, FSurfaceFacet& Facet,TArray<FTransSample> lightingInfo )
		...
		if( Viewport->ColorBytes==2 )
		{
			...
			for( int i=Facet.Span->StartY; i<Facet.Span->EndY; i++ )
			{
				for( FSpan* Span = Facet.Span->Index[i-Facet.Span->StartY]; Span; Span=Span->Next )
				{
					...
					// Gives the flatshaded polys a black outline - looks nice in the editor
					Line.PtrWORD[Span->Start]= 0; // From RUNE : 10/16/00
					Line.PtrWORD[Span->End]  = 0; // From RUNE : 10/16/00
				}
		...
		else if( Viewport->ColorBytes==4 )
		{
			...
			for( int i=Facet.Span->StartY; i<Facet.Span->EndY; i++ )
			{
				for( FSpan* Span = Facet.Span->Index[i-Facet.Span->StartY]; Span; Span=Span->Next )
				{
					...
					// Gives the flatshaded polys a black outline - looks nice in the editor
					Line.PtrDWORD[Span->Start]= 0; // From RUNE : 10/16/00
					Line.PtrDWORD[Span->End]  = 0; // From RUNE : 10/16/00
				}
				Line.PtrBYTE += GByteStride;
			}
		}
