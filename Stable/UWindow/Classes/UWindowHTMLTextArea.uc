						...
						out HTMLStyle StartStyle, out HTMLStyle CurrentStyle)
{
	...
	switch(strTag)
	{
		...
		case "P":
			...
			if(IsValidString(strOption) && strOption == "CENTER")  {
				Log("TIM: Centertin paragraph");
			...
			{
				rowText = Super.AddText(strOutput $ strLeft);
				strOutput = "";
				UWindowHTMLTextRow(rowText).StartStyle = StartStyle;
				UWindowHTMLTextRow(rowText).EndStyle = CurrentStyle;
			}
			StartStyle = CurrentStyle;
			rowText = Super.AddText("");
			UWindowHTMLTextRow(rowText).StartStyle = StartStyle;
			UWindowHTMLTextRow(rowText).EndStyle = CurrentStyle;
			break;
		case "BR":
			strOutput = strOutput $ strLeft;
			rowText = Super.AddText(strOutput);
			
			//TLW: Added link creation if it has the correct text			
			StartStyle.bLink = CheckTextForLink(strOutput);
			if(StartStyle.bLink)  {
			//	Log("TIM: Changed LinkDest to = " $ strOutput);
		...
		case "FONT":
			...
			//	Log("TIM: ending FONT, string=" $ strOutput $
			//		", HTML=" $ strHTML
			//	);
				CurrentStyle.TextColor = TextColor;
				CurrentStyle.BGColor = BGColor;
				...
				strOutput = strOutput $ strLeft;	// $ strHTML;
			//	Log("TIM: beginning FONT, string=" $ strOutput $
			//		", HTML=" $ strHTML
			//	);
...
function bool CheckTextForLink(string strIn)
{
	...
	//	Log("TIM: string is a http link - " $ strIn);
		return true;
	}
	if(Left(strIn, 6) ~= "ftp://")  {
	//	Log("TIM: string is a ftp link - " $ strIn);
		return true;
	}
