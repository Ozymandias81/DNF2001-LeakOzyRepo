...
typedef void (*TFPaletteFunction)(TFAnimation *self, TFUByte (*palette)[256][3]);
...
struct TFAnimationInfo
{
   ...
   TFStatus LoopFlag;         /* TF_TRUE if animation is to be looped
                                 (default is TF_FALSE) */
   unsigned long Speed;       /* Speed in 60th's of a second to delay */
   unsigned char paletteChanged; /* Palette changed last frame. */
};
