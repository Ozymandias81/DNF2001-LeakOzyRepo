...
public:
	...
	INT				ProjectorFrame;	// 4  == GFrameStamp if ProjectorFlags are valid
	DWORD			ProjectorFlags;	// 4  Up to 32 visible projectors on this node

	// Functions.
	UBOOL __forceinline IsCsg( DWORD ExtraFlags=0 ) const
	{
...
public:
	...
	// DNF Surface extensions:
	QWORD PolyFlags2;			// PolyFlags2! Over 2x the flags of the original!
	FName SurfaceTag;			// Tag of this surface

	// JEP: Projector stuff.  Supports up to 32 projectors
	//	(these are not saved out, and are only used during run-time optimizations)
	INT			ProjectorFrame;	// == GFrameStamp if valid for this frame
...
/* Flags describing effects and properties of a Bsp polygon
   ...
   NOTE: If you add or remove a PolyFlag or PolyFlagEx, please add a corresponding REGISTER_POLYFLAG in UnobjEngine.cpp.
*/
...
{
	// Regular in-game flags.
	PFX_AlphaMap		    = 0x00000001,	// NJS: Use alphamap.
	PFX_Clip				= 0x00000002,   // NJS: Poly level clipping
	PFX_DepthFog			= 0x00000004,   // NJS: Hardware depth fog.
	PFX_FlatShade			= 0x00000008,   // NJS: Flat shade, (default is gouraud)
	PFX_LightenModulate		= 0x00000010,
	PFX_DarkenModulate		= 0x00000020,
	PFX_Translucent2		= 0x00000040,
	PFX_MirrorHorizontal	= 0x00000080,
	PFX_MirrorVertical		= 0x00000100,

	PFX_NoOcclude			= PFX_LightenModulate | PFX_DarkenModulate | PFX_Translucent2,
	PFX_All					= 0xFFFFFFFF
};

FString ENGINE_API GetPolyFlagsString(DWORD PolyFlags=0, DWORD PolyFlagsEx=0);
...
public:
	...
	// DNF Surface extensions:
	QWORD PolyFlags2;				// PolyFlags2! Over 2x the flags of the original!
	FName SurfaceTag;				// The name of this surface tag.
	...
	FBox GetBoundingBox		( FVector InLocation );
...
public:
	...
	FVector TransformedVerts[4];
	ADecal*	Actor;			// ADecal actor associated with this decal.
	TArray<INT> Nodes;		// The FBspNode indices on which this decal lies.
	UTexture *Texture;		// Decal's Texture
	FLOAT	DrawScale;		// Decal's Draw Scale
	FLOAT	AmbientGlow;
	FLOAT	ScaleGlow;
	DWORD	PolyFlags;
	FLOAT   SpawnTime;			// When this decal was spawned.
	FLOAT	LastRenderedTime;	// Last time that this decal was rendered.
	FVector Location;
	INT     Behavior;
	FLOAT	BehaviorArgument;
	FRotator BaseRotation;
	DWORD	PolyFlagsEx;		// The decal's extended poly flags.
...
/*-----------------------------------------------------------------------------
	Material structures:
-----------------------------------------------------------------------------*/
struct FDamageCategoryEffectStruct
{
	UClass *HitEffect;		// Effect when a bullet hits this surface.
	UClass *HitSpawn;		// Actor to spawn when a bullet hits.
	USound *HitSounds[8];	// Sounds to randomly choose from when a bullet hits.
	FName   HitTrigger;		// Event to trigger.
};
