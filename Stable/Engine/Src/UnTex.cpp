...
FPlane ENGINE_API FGetHSV( BYTE H, BYTE S, BYTE V )
{
	...
	Brightness  = Clamp(Brightness,(FLOAT)0.f,(FLOAT)1.f);
...
//
void __fastcall UTexture::Tick( FLOAT DeltaSeconds )
{
	// Prime the texture.
...
#if DNF
	// Convert old style material hard references to fnames:
	if(Material)
	{
		if(MaterialName==NAME_None)
			MaterialName=Material->GetFName();
		else
			Material=NULL;
	}
...
// NJS - some collision checking primitives:
UBOOL UTexture::CollisionCheck( int x, int y, UTexture *Other, int oX, int oY )
{
	int left, top, right, bottom, oLeft, oRight, oTop, oBottom;
	int xIndex, yIndex;

	guard(UTexture::CollisionCheck);
	if(!Other) return false;

	// Compute my rectangle:
	left=x; right=left+USize;
	top=y;  bottom=top+VSize;

	// Compute other rectangle:
	oLeft=oX; oRight =oLeft+Other->USize;
	oTop =oY; oBottom=oTop+Other->VSize;

	// See if the bitmaps miss each other completely:
	if(right<oLeft) return false;
	if(left>oRight) return false;
	if(bottom<oTop) return false;
	if(top>oBottom) return false;

	// Ok, so we overlap, now determine if any non-masking in the overlapping region collide :

	
	BYTE *myTex   =(BYTE *)&Mips(0).DataArray(0)/*&GetMip(0)->DataArray(0)*/,		*myIndex;
	BYTE *otherTex=(BYTE *)&Other->Mips(0).DataArray(0), *otherIndex;
	
	for(yIndex=Max(top,oTop);yIndex<Min(bottom,oBottom);yIndex++)
	{
		// Compute this row:
		myIndex   =myTex   +(USize*(yIndex-top))+(Max(left,oLeft)-left);
		otherIndex=otherTex+(Other->USize*(yIndex-oTop))+(Max(left,oLeft)-oLeft);

		for(xIndex=Max(left,oLeft);xIndex<Min(right,oRight);xIndex++)
		{
			if(*myIndex&&*otherIndex) return true;	// Found a colliding pixel.
			myIndex++; otherIndex++;				// Move to next pixel in this column.
		}
	}
...
BYTE UTexture::GetPixel( int x, int y )
{
	if((x<0)||(x>=USize)) return 0;
	if(y<0||(y>=VSize)) return 0;

	return *(((BYTE *)&Mips(0).DataArray(0))+(y*USize)+x);
}

void UTexture::execCollisionCheck( FFrame& Stack, RESULT_DECL )
{
	guard(UTexture::execCollisionCheck);
	
	// Snag the parameters:
	//P_THIS(UTexture);
	P_GET_INT(x);
	P_GET_INT(y);
	P_GET_OBJECT(UTexture,Other);
	P_GET_INT(oX);
	P_GET_INT(oY);
	P_FINISH;

	// Set the result:
	*(UBOOL*)Result = /*This->*/CollisionCheck(x,y,Other,oX,oY);
	
	unguardexec;
}
IMPLEMENT_FUNCTION( UTexture, INDEX_NONE, execCollisionCheck );

void UTexture::execGetPixel( FFrame& Stack, RESULT_DECL )
{
	guard(UTexture::execGetPixel);
	...
	// Set the result:
	*(BYTE*)Result = /*This->*/GetPixel(x,y);
	
	unguardexec;
}
IMPLEMENT_FUNCTION( UTexture, INDEX_NONE, execGetPixel );
