...
void FMovingBrushTracker::Update( AActor* InActor )
{
	...
	for( Link=GroupActors; Link; Link=Link->Next )
	{
		...
		*iActorNodePrevLink = INDEX_NONE;
		...
		// Transform the decals.
		for( i=0; i<LevelSurfs.Num(); i++ )
		{
			FBspSurf* Surf = LevelSurfs(i);
			for( INT j=0; j<Surf->Decals.Num(); j++ )
			{
				FDecal* Decal = &Surf->Decals(j);
				FCoords DecalCoords = GMath.UnitCoords * (Actor->Rotation - Decal->BaseRotation);
				for( INT k=0; k<4; k++ )
				{
					Decal->TransformedVerts[k] = Decal->Vertices[k].TransformVectorBy( DecalCoords );
				}
			}
		}
...
void FMovingBrushTracker::SetupActorBrush( AMover* Actor )
{
	...
	for( INT i=0; i<Brush->Polys->Element.Num(); i++ )
	{
		...
		// DNF Extensions:
		Surf->PolyFlags2		  = Poly->PolyFlags2;
		Surf->SurfaceTag		  = Poly->SurfaceTag;
