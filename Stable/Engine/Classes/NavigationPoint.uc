...
// JC: New flag used to determine if this point should be used for cover.
var( AIBehavior ) bool bCoverPoint ?("If true, the node will be flagged as a Cover Point.");
var( AIBehavior ) bool bDuckPoint ?("If true, the node will be flagged as a Duck Point.");
var( AIBehavior ) bool bSnipePoint;
var( AIBehavior ) bool bExitOnCantSee ?("If false, the NPC will remain at Cover Point even after line of\nsight is lost. No hunting occurs.");
var( AIBehavior ) bool bExitOnDistance ?("If true, Covering NPC will hunt enemy once enemy is passed the\nExitDistance threshhold.");
var( AIBehavior ) float ExitDistance ?("Distance (in units) enemy needs to be away before Covering NPC begins hunt.");
var( AIBehavior ) float OffsetDistance ?("Distance NPC can be from this point\nfor it to still be considered reached.");
var( AIBehavior ) bool bExitWhenClose ?("NPC will abandon a Cover or Duck Point when enemy is too close.");
var( Events ) name BreakConnectTag;
