...
var() class<DamageType> TriggerDamageType ?("Damage type to trigger on.");
var() bool       bDynamicLightMover; // Apply dynamic lighting to mover.
var() bool		 bTranslucentMover; // If this is a mover that should be seen through.
var() int		 MoverThickness;	// Talk to Jess before changing.
...
var() bool		 bDontDuckOnEncroach; // Player won't duck when it hits him.
...
native final function GetMoverCollisionBox(out vector Min, out vector Max);
...
function bool EncroachingOn( actor Other )
{
	...
	{
		Other.TakeDamage( 10000, None, Other.Location, vect(0,0,0), class'CrushingDamage' );
		return false;
	}
	...
	if( EncroachDamage != 0 )
		Other.TakeDamage( EncroachDamage, Instigator, Other.Location, vect(0,0,0), class'CrushingDamage' );
	...
	else if( MoverEncroachType == ME_CrushWhenEncroach )
	{
		...
		if ( Other.bIsPawn )
		{
			Pawn(Other).Health = -100;
			Pawn(Other).Died( Instigator, class'CrushingDamage', Location );
		}
		return false;
	}
...
{
	if ( (TriggerDamageType != None) && (DamageType != TriggerDamageType) )
		return;
...
// When triggered and we don't want any special movement
state() TriggerNoMove
{
