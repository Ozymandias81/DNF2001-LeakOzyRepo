class UDukeInGameWindowClasses expands UDukeInGameWindow;
...
function Created()
{
	...
	{
		if ( Team == 0 )
			OptionButtons[i].Text = V.default.HumanTeamClassNames[i] @ "(" $ V.default.HumanTeamCost[i] $ ")";
		else if ( Team == 1 )
			OptionButtons[i].Text = V.default.BugTeamClassNames[i] @ "(" $ V.default.BugTeamCost[i] $ ")";
	}
...
//			GetPlayerOwner().PlaySound(sound'SpeechWindowClick', SLOT_Interact);
			for ( i=0; i<NumOptions; i++ )
			{
				...
				{
					Root.GetPlayerOwner().ChangeClass( Left( OptionButtons[i].Text, Len(OptionButtons[i].Text) - 4 ) );
					Log( Left( OptionButtons[i].Text, Len(OptionButtons[i].Text) - 4 ) );
				}
			}
			if (B == TopButton)
			{
