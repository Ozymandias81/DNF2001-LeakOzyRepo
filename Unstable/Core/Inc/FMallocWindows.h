...
public:
	...
	void DumpAllocsStat(FSceneNode* Frame)
	{
		Frame=Frame;
	}

	TCHAR *GetAllocsStats() 
	{
		static TCHAR AllocStats[8192];
		TCHAR TempBuffer[8192];
		AllocStats[0]=0;

		appStrcat(AllocStats,TEXT("FMallocWindows Status: "));

		appSprintf(TempBuffer,TEXT("Curr Memory: %5.3fM / %5.3fM, "),UsedCurrent/1024.0/1024.0, OsCurrent/1024.0/1024.0 );
		appStrcat(AllocStats,TempBuffer);
		appSprintf(TempBuffer,TEXT("Peak Memory: %5.3fM / %5.3fM, "), UsedPeak   /1024.0/1024.0, OsPeak   /1024.0/1024.0 );
		appStrcat(AllocStats,TempBuffer);
		appSprintf(TempBuffer,TEXT("Allocs: %6i Current / %6i Total, "), CurrentAllocs, TotalAllocs );
		appStrcat(AllocStats,TempBuffer);
