class UDukeInGameWindowSpeech expands UDukeInGameWindow;
...
function Created()
{
	...
	local class<dnVoicePack>   		V;

	V = class<dnVoicePack>( GetPlayerOwner().PlayerReplicationInfo.VoiceType );
	
	if ( V == None )
		Log("UDukeInGameWindowSpeech: Critical error, V is none.");
	...
	CurrentType = UDukeInGameWindow(ParentWindow).CurrentType;
