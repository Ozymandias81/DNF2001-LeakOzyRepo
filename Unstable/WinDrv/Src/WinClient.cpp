...
//		||	appStricmp(GConfig->GetStr(TEXT("Engine.Engine"),TEXT("GameRenderDevice")),TEXT("GlideDrv.GlideRenderDevice"))==0
	...
	if ( !GIsEditor )
	{
		for( ; ; )
		{
			...
			CoInitialize( NULL );
			HRESULT hr = CoCreateInstance( CLSID_DirectInput8, NULL, CLSCTX_INPROC_SERVER, IID_IDirectInput8, (void**)&DirectInput );
			if ( hr != DI_OK )
			{
				debugf( NAME_Init, TEXT("DirectInput8 not installed. (Failed to acquire the interface.)") );
				break;
			}
			hr = DirectInput->Initialize( hInstance, DIRECTINPUT_VERSION );
			if ( FAILED( hr ) )
			{
				debugf( NAME_Init, TEXT("Failed to initialize the DirectInput object.") );
				diShutdown();
   				break;
			}
	...
	// Success.
	debugf( NAME_Init, TEXT("Win32 client initialized.") );
}
