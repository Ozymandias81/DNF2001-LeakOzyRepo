...
)
{
	...
	if( !LODInfo->LevelOfDetail )
		Mesh = new( InParent, MeshName, RF_Public|RF_Standalone )UUnrealMesh( JSDataHdr.NumPolys, JSDataHdr.NumVertices, JSAnivHdr.NumFrames );
	else 
		Mesh = new( InParent, MeshName, RF_Public|RF_Standalone )UUnrealLodMesh( JSDataHdr.NumPolys, JSDataHdr.NumVertices, JSAnivHdr.NumFrames );
	...
	if( LODInfo->LevelOfDetail )
		meshLODProcess( (UUnrealLodMesh*)Mesh, LODInfo );
...
//
void UEditorEngine::meshBuildBounds( UUnrealMesh* Mesh )
{
