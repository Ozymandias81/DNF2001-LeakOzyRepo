...
boolean OMenuItem::OnPress(inputevent_t* event)
{
...
boolean OMenuItem::OnRelease(inputevent_t* event)
{
...
void OMenu::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	vid.FlatColor(255, 255, 255);
	vid.DrawString(sx+2, sy+1, 8, 8, name, true, 128, 128, 128);
    vid.DrawString(sx+dim.x-8, sy+1, 8, 8, ">", true, 128, 128, 128);
}

boolean OMenu::OnPress(inputevent_t* event)
{
	if (event->key != KEY_MOUSELEFT)
		return(Super::OnPress(event));
    if ((event->mouseX < 0) || (event->mouseX > dim.x))
...
boolean OMenu::OnRelease(inputevent_t* event)
{
	OVL_UnlockInput(this);
    return(1);
}

boolean OMenu::OnPressCommand(int argNum, char **argList)
{
    OVLCMDSTART
    OVLCMD("MenuKill")
    {
...
void OCheckBoxControl::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	vid.FlatColor(255, 255, 255);
	p[0].Set(sx, sy, 0);
	p[1].Set(sx+10, sy+10, 0);
	vid.DrawLineBox(&p[0], &p[1], NULL, NULL, false);
	vid.DrawString(sx+12, sy+2, 8, 8, name, true, 128, 128, 128);
    switch(checked)
    {
    case 0: /*vid.DrawString(sx+1, sy+2, 8, 8, "X", true, 255, 0, 0);*/ break;
    case 1: vid.DrawString(sx+1, sy+2, 8, 8, "X", true, 0, 255, 0); break;
    case 2: vid.DrawString(sx+1, sy+2, 8, 8, "/", true, 64, 64, 64); break;
    }
}

boolean OCheckBoxControl::OnPress(inputevent_t* event)
{
	if (event->key != KEY_MOUSELEFT)
		return(Super::OnPress(event));
...
boolean OCheckBoxControl::OnRelease(inputevent_t* event)
{
	OVL_UnlockInput(this);
    return(1);
}
...
boolean OSpinBoxControl::OnPress(inputevent_t* event)
{
	if (event->key != KEY_MOUSELEFT)
		return(Super::OnPress(event));
...
boolean OSpinBoxControl::OnDrag(inputevent_t* event)
{
	if (event->key != KEY_MOUSELEFT)
...
boolean OSpinBoxControl::OnRelease(inputevent_t* event)
{
	OVL_UnlockInput(this);
    return(1);
}
...
//	OVL_SetRedraw(this, false);
	...
	if (dim.x < 160+6)
		dim.x = 160+6;
	dim.y = tooldy + 12+6+10;
	
	vid.ColorMode(VCM_FLAT);
	vid.FlatColor(255, 255, 255);
	p[0].Set(sx, sy+dy-9, 0);
	p[1].Set(sx+dx, sy+dy-9, 0);
	...
	for (tool=toolsHead->next; tool!=toolsHead; tool=tool->next)
	{
		...
		else
		{
			...
			p[0].Set(sx+checkx, sy+checky, 0);
			p[1].Set(sx+checkx+16, sy+checky, 0);
			p[2].Set(sx+checkx+16, sy+checky+16, 0);
			p[3].Set(sx+checkx, sy+checky+16, 0);
			...
			if ((tool->active) || (tool->flashframes))
			{
				...
				vid.FlatColor(tool->color.x, tool->color.y, tool->color.z);
				vid.FlatAlpha(96);
	...
	if (mtool)
	{
		tsize = (dim.x - 6) / strlen(mtool->name);
		if (tsize > 8)
			tsize = 8;
...
boolean OToolbar::OnPress(inputevent_t *event)
{
	...
	case OVLTOOL_INSTANT:
		CON->Execute(ovl_focusOverlay, tool->commands[0], 0);
		tool->flashframes = 0.1 / sys_frameTime; // flash for 1/10th second
		break;
	}
	return(1);
}
...
/*
boolean OToolbar::OnDrag(inputevent_t *event)
{
}
*/

boolean OToolbar::OnRelease(inputevent_t *event)
{
	OVL_UnlockInput(this);
	if (ovl_focusOverlay)
...
/*
boolean OToolbar::OnPressCommand(int argNum, char **argList)
{
}
*/

/*
boolean OToolbar::OnDragCommand(int argNum, char **argList)
{
}
*/

/*
boolean OToolbar::OnReleaseCommand(int argNum, char **argList)
{
}
*/

/*
boolean OToolbar::OnMessage(ovlmsg_t *msg)
{
}
*/
...
ovltool_t *OToolWindow::ToolForName(char *name)
{
	if (!name)
		return(NULL);
	for (int i=0;i<OTOOLWINDOW_MAXCONTEXTS;i++)
...
void OToolWindow::OnSave()
{
	...
	for (i=0; i<OTOOLWINDOW_MAXCONTEXTS; i++)
	{
		...
		for (tool=contexts[i].toolsHead.next; tool!=&contexts[i].toolsHead; tool=tool->next)
		{
			...
			VCR_WriteByte(tool->active);
			VCR_WriteByte((byte)tool->tooltype);
...
/*
boolean OToolWindow::OnPress(inputevent_t *event)
{
}
*/

/*
boolean OToolWindow::OnDrag(inputevent_t *event)
{
}
*/

/*
boolean OToolWindow::OnRelease(inputevent_t *event)
{
}
*/

boolean OToolWindow::OnPressCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("radiocmd")
	{
...
//			ActivateRadioTool(tool);
	...
	OVLCMD("toolcolor")
	{
		...
		if (!(tool = ToolForName(argList[1])))
			return(1);
		tool->color.Set(atoi(argList[2]), atoi(argList[3]), atoi(argList[4]));
		return(1);
	}
	OVLCMD("toolactivate")
	{
...
boolean OToolWindow::OnDragCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("radiocmd")
	{
...
boolean OToolWindow::OnReleaseCommand(int argNum, char **argList)
{
	OVLCMDSTART
	OVLCMD("radiocmd")
	{
...
boolean OToolWindow::OnMessage(ovlmsg_t *msg)
{
	OVLMSGSTART
...
void OSelectionBox::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	for (i=0;i<numItems;i++)
	{
		...
		{
			p[0].Set(sx, sy+i*8 - adjy, 0);
			p[2].Set(sx+dx, p[0].y+8, 0);
...
boolean OSelectionBox::OnPress(inputevent_t *event)
{
	int i, k, adjy;
	char *ptr;
	...
	if (event->key != KEY_MOUSELEFT)
	{
		...
		{			
			...
			else
			{
				...
				for (k=0,ptr=itemStr;k<numItems;k++,ptr+=strlen(ptr)+1)
				{
	...
	ptr = itemStr;
	for (i=0,ptr=itemStr;i<selectedItem;i++,ptr+=strlen(ptr)+1)
		;
...
/*
boolean OSelectionBox::OnDrag(inputevent_t *event)
{
}
*/

/*
boolean OSelectionBox::OnRelease(inputevent_t *event)
{
}
*/

/*
boolean OSelectionBox::OnPressCommand(int argNum, char **argList)
{
}
*/

/*
boolean OSelectionBox::OnDragCommand(int argNum, char **argList)
{
}
*/

/*
boolean OSelectionBox::OnReleaseCommand(int argNum, char **argList)
{
}
*/

boolean OSelectionBox::OnMessage(ovlmsg_t *msg)
{
	OVLMSGSTART
	OVLMSG("SelectionBoxInfo") // char *choices, overlay_t *cbOvl, char *cbCommand, int multiSelect
	{
		...
		numItems = 0;
		len = 1;
		for (ptr=OVLMSGPARM(0, char *); *ptr; ptr += strlen(ptr)+1)
		{
			len += strlen(ptr)+1;
			numItems++;
		}
		itemFlags = ALLOC(byte, numItems);
...
void OInputBox::OnDraw(int sx, int sy, int dx, int dy, ovlClipBox_t *clipbox)
{
	...
	p[0].Set(sx+2, sy+12, 0);
	p[1].Set(sx+dx-2, p[0].y+8, 0);
	vid.ColorMode(VCM_FLAT);
	...
	len = ((dx-8)/6)-1;
	start = 0;
	if ((int)strlen(inputBuffer)>len)
		start = strlen(inputBuffer)-len;
	vid.DrawString(sx+4, sy+14, 6, 6, inputBuffer+start, true, 128, 128, 128);
	if ((int)(sys_curTime*4.0) & 1)
		vid.DrawString(sx+4+strlen(inputBuffer+start)*6, sy+13, 6, 6, "_", true, 128, 128, 128);
}

boolean OInputBox::OnPress(inputevent_t *event)
{
	int key;
	char tempstr[32];
	...
	case KEY_BACKSPACE:
		...
		break;
	default:
		if ((vid.charDrawable[event->key]) // is it drawable?
...
/*
boolean OInputBox::OnDrag(inputevent_t *event)
{
}
*/

/*
boolean OInputBox::OnRelease(inputevent_t *event)
{
}
*/

/*
boolean OInputBox::OnPressCommand(int argNum, char **argList)
{
}
*/

/*
boolean OInputBox::OnDragCommand(int argNum, char **argList)
{
}
*/

/*
boolean OInputBox::OnReleaseCommand(int argNum, char **argList)
{
}
*/

boolean OInputBox::OnMessage(ovlmsg_t *msg)
{
	OVLMSGSTART
