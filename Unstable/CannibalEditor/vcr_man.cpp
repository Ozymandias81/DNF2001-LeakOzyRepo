...
static int WriteByte(action_t *action, byte val)
{
	if (!action)
...
static int WriteShort(action_t *action, short val)
{
	...
	if (action->writeindex >= (action->datalen-1))
		return(0);
	*((short *)((byte *)action->data+action->writeindex)) = val;
	action->writeindex += 2;
	return(1);
}
...
static int WriteInt(action_t *action, int val)
{
	...
	if (action->writeindex >= (action->datalen-3))
		return(0);
	*((int *)((byte *)action->data+action->writeindex)) = val;
	action->writeindex += 4;
	return(1);
}
...
static int WriteFloat(action_t *action, float val)
{
	...
	if (action->writeindex >= (action->datalen-3))
		return(0);
	*((float *)((byte *)action->data+action->writeindex)) = val;
	action->writeindex += 4;
	return(1);
}
...
static int WriteBulk(action_t *action, void *data, int len)
{
	...
	if (action->writeindex >= (action->datalen-(len-1)))
		return(0);
	if (data)
		memcpy((byte *)action->data+action->writeindex, data, len);
	else
		memset((byte *)action->data+action->writeindex, 0, len);
...
static byte ReadByte(action_t *action)
{
	if (!action)
		SYS_Error("ReadByte: No open action");
	if (action->flags & AF_READBACKWARD)
...
static short ReadShort(action_t *action)
{
	...
	if (action->flags & AF_READBACKWARD)
	{
		...
		return(*((short *)((byte *)action->data+action->readindex)));
	}
	else
	{
		if (action->readindex >= (action->writeindex-1))
			SYS_Error("ReadInt: Overflow in \"%s\"", action->sig);
		action->readindex += 2;
		return(*((short *)((byte *)action->data+action->readindex-2)));
	}
}
...
static int ReadInt(action_t *action)
{
	...
	if (action->flags & AF_READBACKWARD)
	{
		...
		if (action->readindex < 0)
			SYS_Error("ReadInt: Overflow in \"%s\"", action->sig);
		return(*((int *)((byte *)action->data+action->readindex)));
	}
	else
	{
		if (action->readindex >= (action->writeindex-3))
			SYS_Error("ReadInt: Overflow in \"%s\"", action->sig);
		action->readindex += 4;
		return(*((int *)((byte *)action->data+action->readindex-4)));
	}
}
...
static float ReadFloat(action_t *action)
{
	...
	if (action->flags & AF_READBACKWARD)
	{
		...
		return(*((float *)((byte *)action->data+action->readindex)));
	}
	else
	{
		if (action->readindex >= (action->writeindex-3))
			SYS_Error("ReadFloat: Overflow in \"%s\"", action->sig);
		action->readindex += 4;
		return(*((float *)((byte *)action->data+action->readindex-4)));
	}
}
...
void VCR_Record(vcractiontype_t vt, const char *nsig, void (*nundofunc)(char *), int bufsize, byte **localPtr)
{
	...
	case VCRA_UNDO:
		...
		if (temp->next)
		{
			if (temp->next->data) FREE(temp->next->data);
			FREE(temp->next);
			temp->next = NULL;
		}
		break;
...
void VCR_EnlargeActionDataBuffer(int addbufsize)
{
	if (!vcr_activeAction)
		return;
	byte *odata = vcr_activeAction->data;
	vcr_activeAction->datalen += addbufsize;
	vcr_activeAction->data = ALLOC(byte, vcr_activeAction->datalen);
...
int VCR_WriteString(char *str)
{
	if (!str)
		return(0);
	return(WriteBulk(vcr_activeAction, str, strlen(str)+1));
}
