...
class ENGINE_API UCanvas : public UObject
{
	...
	class URender    * Render; // Only valid during UGameEngine::Draw
	TMap<FString, UFont*> LoadedTTFonts;

	// UCanvas interface.
	virtual void Init( UViewport* InViewport );
	virtual void Update( FSceneNode* Frame );
	virtual void DrawTile( UTexture* Texture, 
							FLOAT X, FLOAT Y, 
							FLOAT XL, FLOAT YL, 
							FLOAT U, FLOAT V, 
							FLOAT UL, FLOAT VL, 
							class FSpanBuffer* SpanBuffer, 
							FLOAT Z, 
	...
	virtual void DrawIcon( UTexture* Texture, FLOAT ScreenX, FLOAT ScreenY, FLOAT XSize, FLOAT YSize, class FSpanBuffer* SpanBuffer, FLOAT Z, FPlane Color, FPlane Fog, DWORD PolyFlags, DWORD PolyFlagsEx=0, UBOOL Bilinear=0, FLOAT Alpha=1.0, FLOAT rot=0.0 );
	virtual void DrawPattern( UTexture* Texture, FLOAT X, FLOAT Y, FLOAT XL, FLOAT YL, FLOAT Scale, FLOAT OrgX, FLOAT OrgY, class FSpanBuffer* SpanBuffer, FLOAT Z, FPlane Color, FPlane Fog, DWORD PolyFlags, DWORD PolyFlagsEx=0 );
	virtual void DrawLine(FVector P1, FVector P2, UBOOL Is3D);
...
private:
	// Internal functions.
	void VARARGS WrappedPrint( ERenderStyle Style, INT& XL, INT& YL, UFont* Font, UBOOL Center, const TCHAR* Text, UBOOL Clip=0, FLOAT XScale=1.0, FLOAT YScale=1.0 );
};
...
enum EViewportShowFlags
{
	...
	SHOW_HardwareBrushes= 0x00008000,	// Show hardware brushes.
};
...
class ENGINE_API UViewport : public UPlayer
{
	...
	virtual UBOOL Lock( FColor FogColor, float FogDensity, INT FogDistance, FPlane FlashScale, FPlane FlashFog, FPlane ScreenClear, DWORD RenderLockFlags, BYTE* HitData=NULL, INT* HitSize=NULL );
	...
	virtual void TryRenderDevice( const TCHAR* ClassName, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen )=0;

	// Brandon's camera portal stuff.
	AActor* GetPortalView( FName& PortalName );
...
class ENGINE_API UClient : public UObject
{
	...
	// JEP...
	// Detail explanation
	//	ShadowDetail will override the advanced settings, unless it's set to 5 (Custom)
	//	0 = no Shadows, 1 = 8/1 shadows, 2 = 8/1 shadows filtered, 3 = 16/2 shadows, 4 = 16/2 shadows filtered, 5 = Custom
	INT			ShadowDetail;			
	// Custom shadow settings
	UBOOL		EnableShadows;
	INT			MaxShadows;
	INT			MaxShadowsPerActor;
	UBOOL		FilterShadows;
	// ...JEP
