...
void UNetConnection::ReceivedRawPacket( void* InData, INT Count )
{
	...
	debugfSlow( NAME_DevNetTraffic, TEXT("***%03i: Packet Received %i***"), (INT)(appSeconds()*1000)%1000, Count );
...
void UNetConnection::ReceivedPacket( FBitReader& Reader )
{
	...
	while( !Reader.AtEnd() && State!=USOCK_Closed )
	{
		...
		if( IsAck )
		{
			...
			{
				LagAcc += Driver->Time - OutLagTime[Index];
				LagCount++;
			}
...
{
	debugfSlow( NAME_DevNetTraffic, TEXT( "SendAck %i" ), AckPacketId );
...
UChannel* UNetConnection::CreateChannel( EChannelType ChType, UBOOL bOpenedLocally, INT ChIndex )
{
	...
	debugfSlow( NAME_DevNetTraffic, TEXT("Created channel %i of type %i"), ChIndex, ChType);
...
{
	// NOTE: This function can be called in DukeForever to change the 
	// viewport to a new pawn.  
