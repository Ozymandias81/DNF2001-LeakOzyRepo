...
var bool		bAlphaOnly;		//TLW: Added for DukeNet input, AlphaNumeric by turning both on
...
// (JP)		Build the protected value that we will render
	...
	if(bCanEdit && bKeyDown)
	{
		if( !bControlDown )
		{
			...
			if(bNumericOnly && bAlphaOnly)  {
				// keys like ":" in the range 0x3A-0x3F are reserved for DukeNet commands
				if( Key >= 0x30 && Key <= 0x39 ||
					Key >= 0x40 && Key <  0x80 )
					Insert(Key);
			}
			else if(bNumericOnly)  {
				if( Key >= 0x30 && Key <= 0x39 )  
					Insert(Key);
			}
			else if(bAlphaOnly)  {
...
function Paint( Canvas C, float X, float Y )
{
	...
	if ( (W > WinWidth) && (Font != F_Small) && !bNoShrinkFont )
	{
//		if ( bComboBox )
//			YOff = 1;
...
//		YOff += 2;
	...
	// (JP)
	if (bProtectValue)
		ClipText(C, Offset + 1, TextY+YOff,  ProtectedValue);
	else
		ClipText(C, Offset + 1, TextY+YOff,  Value);
	...
	if ( bShowCaret )
		ClipText( C, Offset + W - 1, TextY+YOff, "|" );
}
