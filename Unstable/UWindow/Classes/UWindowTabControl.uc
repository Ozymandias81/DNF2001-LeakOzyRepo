...
function Created()
{
	...
	TabArea = UWindowTabControlTabArea(CreateWindow(class'UWindowTabControlTabArea', 
					0, 0, WinWidth - 10 - (LookAndFeel.SBPosIndicator.W * 2),
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	if(bMultiLine)
		...
		TabArea.WinWidth = 	WinWidth - 10 -
							(LookAndFeel.SBPosIndicator.W * 2);
...
function Paint(Canvas C, float X, float Y)
{
	...
	DrawStretchedTextureSegment( C, 
								 0, 0, 
								 WinWidth, LookAndFeel.Size_TabAreaHeight * TabArea.TabRows, 
								 R.X, R.Y, 
								 R.W, R.H, 
								 T
	);
