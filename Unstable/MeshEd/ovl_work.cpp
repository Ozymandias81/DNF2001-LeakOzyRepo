...
CONFUNC(SaveProject, NULL, 0)
{
	...
	if (argNum < 2)
	{
		if (!(str = allocd_str = (char *)SYS_SaveFileBox("Duke Nukem Extended Model (*.mdx)\0*.mdx\0\0", "Save MDX", "mdx")))
			return;
	}
...
/* delayed load project */
U32 load_project(ConQueue *q);
/* queue till after frame is done */
CONFUNC(LoadProject, NULL, 0)
{
	mesh_app.QueueConsoleAction(argNum,(CC8 **)argList,load_project);
}
...
U32 load_project(ConQueue *q)
{
	...
	if (argNum < 2)
	{
		if (!(str = allocd_str = (char *)SYS_OpenFileBox("Duke Nukem Extended Model (*.mdx)\0*.mdx\0\0", "Open MDX", "mdx")))
			return FALSE;
	}
	ws = (OWorkspace *)ovl_Windows;
...
/*
U32 OWorkspace::OnPress(inputevent_t *event)
{
}
*/

/*
U32 OWorkspace::OnDrag(inputevent_t *event)
{
}
*/

/*
U32 OWorkspace::OnRelease(inputevent_t *event)
{
}
*/

U32 OWorkspace::OnPressCommand(int argNum, CC8 **argList)
{
	OVLCMDSTART
...
//-------------------- OPEN -------------------------	
	OVLCMD("ws_open_skin")
	{
		...
		if (argNum < 3)
		{
			if (!(str = allocd_str = (char *)SYS_OpenFileBox("Image Files (*.bmp,*.tga)\0" "*.bmp;*.tga\0" "24-bit Bitmap Files (*.bmp)\0" "*.bmp\0" "32-bit Targa Files (*.tga)\0" "*.tga\0\0", "Open Skin",null)))
				return(1);
		}
	...
	OVLCMD("ws_open_mrg")
	{
		...
		if (argNum == 1)
		{
			if (!(str = allocd_str = (char *)SYS_OpenFileBox("MRGPlay Data (*.mrp)\0*.mrp\0"
										"\0", "Open MRG Data", "mrp")))
				return(1);
		}
		else
...
//-------------------- NEW --------------------------	
	...
	OVLCMD("ws_new_skin")
	{
		...
		if (!(str = allocd_str = (char *)SYS_SaveFileBox("24-bit Bitmap Files (*.bmp)\0*.bmp\0\0", "Create Skin - Choose a filename", "bmp")))
			return(1);
		U16 *wtemp = ALLOC(U16, width*height);
		memset(wtemp, 0, width*height*sizeof(U16));
...
//-------------------- SPAWN ------------------------	
	...
	OVLCMD("ws_spawnwindow_skin")
	{
		...
		kid->camera.SetPositioni(0, 0, 256);
		kid->camera.SetTarget(0, 0, 0);
		kid->camera.SetViewVolumei(vid->res.width, vid->res.height, 4, 1024);
		kid->camera.SetFOVi(vid->res.width, vid->res.height);
		kid->skin = skin;		
		strcpy(kid->name, SYS_GetFileRoot(kid->skin->name));
		kid->dim.x = (float)(kid->skin->tex->width+6);
		kid->dim.y = (float)(kid->skin->tex->width+6+12);
		if (kid->dim.x < 64)
			kid->dim.x = 64+6;
		if (kid->dim.y < 64)
			kid->dim.y = 64+6+12;
		...
		if (kid->skin->tex->width >= kid->skin->tex->height)
			kid->camera.SetPositioni(0, 0, kid->skin->tex->width);
		else
			kid->camera.SetPositioni(0, 0, kid->skin->tex->height);
		return(1);
	}
	OVLCMD("ws_spawnwindow_frame")
	{
		...
		kid->camera.SetPositioni(0, 0, 128);
		kid->camera.SetTarget(0, 0, 0);
		kid->camera.SetViewVolumei(vid->res.width, vid->res.height, 2, 1024);
...
/*
U32 OWorkspace::OnDragCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OWorkspace::OnReleaseCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OWorkspace::OnMessage(ovlmsg_t *msg)
{
}
*/

U32 OWorkspace::OnDragDrop(overlay_t *dropovl)
{
...
{	
	...
	if ((mode == WRM_FRAMES) && (ws->mdx->frames.Count()))
	{
		...
		if (vpos.y < 0)
			vpos.y = 0;
		vmax.y = (float)(ws->mdx->frames.Count()*12+18);
		vmin.y = 0;
	}
	else
	if ((mode == WRM_SEQUENCES) && (ws->mdx->seqs.Count()))
	{
		...
		if (vpos.y < 0)
			vpos.y = 0;
		vmax.y = (float)(ws->mdx->seqs.Count()*12+18);
		vmin.y = 0;
	}
}
...
//	vid->ClipWindow(sx, sy, sx+dx, sy+dy);
	...
	p[0].Seti(sx, sy+10, 0);
	p[1].Seti(sx+dx, sy+10, 0);
	vid->DrawLine(&p[0], &p[1], NULL, NULL);
	modeWidth = dx / WRM_NUMTYPES;
	for (i=1;i<WRM_NUMTYPES;i++)
	{
		p[0].Seti(sx+modeWidth*i, sy, 0);
		p[1].Seti(sx+modeWidth*i, sy+10, 0);
		vid->DrawLine(&p[0], &p[1], NULL, NULL);
	}
	for (i=0;i<WRM_NUMTYPES;i++)
	{
		...
		if (i == (int)mode)
		{
			p[0].Seti(sx+i*modeWidth, sy, 0);
			p[2].Seti(sx+i*modeWidth+modeWidth-1, sy+9, 0);
		...
		tsize = (modeWidth-4) / fstrlen(wrmNames[i]);
		if (tsize > 8)
			tsize = 8;
	...
	if (mode == WRM_SKINS)
	{
		...
		for (i=0;i<WS_MAXSKINS;i++)
		{
			...
			{
				p[0].Seti(sx, sy+i*12, 0);
				p[2].Seti(sx+dx, sy+i*12+11, 0);
				p[1].Setf(p[2].x, p[0].y, 0.0f);
				p[3].Setf(p[0].x, p[2].y, 0.0f);
				vid->ColorMode(VCM_FLAT);
			...
			p[0].Seti(sx, sy+i*12+12, 0);
			p[1].Seti(sx+dx, sy+i*12+12, 0);
			vid->ColorMode(VCM_FLAT);
			vid->FlatColor(255,255,255);
			vid->DrawLine(&p[0], &p[1], NULL, NULL);
		}
		...
		else
		{
			...
			MRL_ITERATENEXT(f,f,ws->mdx->frames)
			{
				...
				if (f->flags & MRF_SELECTED)
				{
					...
					p[2].Seti(sx+dx, sy+i*12-adjy+11, 0);
					...
					vid->MagFilterMode(VFM_BILINEAR);
					p[0].Seti(sx, sy+i*12-adjy, 0);
					p[2].Seti(sx+11, sy+i*12-adjy+11, 0);
					p[1].Setf(p[2].x, p[0].y, 0);
					p[3].Setf(p[0].x, p[2].y, 0);
				...
				p[1].Seti(sx+dx, sy+i*12-adjy+12, 0);
				vid->ColorMode(VCM_FLAT);
				vid->FlatColor(255,255,255);
				vid->DrawLine(&p[0], &p[1], NULL, NULL);
				i++;
...
U32 OWorkResources::OnPress(inputevent_t *event)
{
	int i, k;
	modelFrame_t *f, *f2;
	modelSequence_t *s, *s2;
	...
	if ((event->mouseY <= 10) && (event->key == KEY_MOUSELEFT))
	{
		...
		modeWidth = (int)((dim.x-6) / WRM_NUMTYPES);
...
/*
U32 OWorkResources::OnDrag(inputevent_t *event)
{
}
*/

U32 OWorkResources::OnRelease(inputevent_t *event)
{
...
/*
U32 OWorkResources::OnPressCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OWorkResources::OnDragCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OWorkResources::OnReleaseCommand(int argNum, CC8 **argList)
{
}
*/

/*
U32 OWorkResources::OnMessage(ovlmsg_t *msg)
{
}
*/
