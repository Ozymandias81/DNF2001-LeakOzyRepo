...
int MDX_LoadChunkFile(mdxChunkFile_t *cf, CC8 *filename, char *tmark, int typevers)
{
	...
	if ((cf->header.marker != (U32)(ASCFMARKER))
	...
	// check user markers
	if ((cf->header.typeMarker != (U32)((tmark[0]<<0)+(tmark[1]<<8)+(tmark[2]<<16)+(tmark[3]<<24)))
...
{
	cf->header.marker = (U32)(ASCFMARKER);
	cf->header.ascfVersion = ASCFVERSION;
	cf->header.typeMarker = (U32)((tmark[0]<<0)+(tmark[1]<<8)+(tmark[2]<<16)+(tmark[3]<<24));
	cf->header.typeVersion = typevers;
...
ascfentrylink_t *MDX_FindChunk(mdxChunkFile_t *cf, ascfentrylink_t *prev, char *label, char *instance, int *len, int *version)
{
	...
	{
		if ((label) &&
			(temp->entry.chunkLabel != (U32)((label[0]<<0)+(label[1]<<8)+(label[2]<<16)+(label[3]<<24))))
			continue;
		if ((instance) &&
...
ascfentrylink_t *MDX_AddChunk(mdxChunkFile_t *cf, char *label, char *instance, int len, U8 version, void *data)
{
	...
	temp = ALLOC(ascfentrylink_t, 1);
	if (label)
		temp->entry.chunkLabel = *((U32 *)label);
	else
		temp->entry.chunkLabel = *((U32 *)"????");
