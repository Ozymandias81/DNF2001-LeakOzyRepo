...
//    Definitions
//----------------------------------------------------------------------------
#define ALLOC(type, num) (type*)xmalloc((num)*sizeof(type))
...
extern "C" CC8 *SYS_GetFilePath(CC8 *filename);
extern "C" char *SYS_GetFileRoot(CC8 *filename);
extern "C" char *SYS_GetFileName(CC8 *filename);
...
public:
	...
	void realloc_argv(void);
	CC8 **get_argv(void){return (CC8 **)((char **)argv);}
};
