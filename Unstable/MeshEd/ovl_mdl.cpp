...
typedef int (*frameImportFunc_t)(CC8 *filename,
...
//		verts[i].mountIndex = 0;
	...
	for (i=0;i<numTris;i++)
	{
		baseTris[i].tverts[0].Seti(-1,-1,0);
		baseTris[i].tverts[1].Seti(-1,-1,0);
		baseTris[i].tverts[2].Seti(-1,-1,0);
		baseTris[i].flags = 0;
		...
		{ // check each old triangle
			...
			if ((match[0] >= 0) && (match[1] >= 0) && (match[2] >= 0))
			{
				...
				{ // copy baseframe coordinates over
					baseTris[i].tverts[0].Setf(pbTri[match[0]*2], pbTri[match[0]*2+1], 0);
					baseTris[i].tverts[1].Setf(pbTri[match[1]*2], pbTri[match[1]*2+1], 0);
					baseTris[i].tverts[2].Setf(pbTri[match[2]*2], pbTri[match[2]*2+1], 0);
...
void modelFrame_t::CalcVertNormals()
{
	...
	for (i=0;i<numVerts;i++)
	{
		vertNorms[i] /= (float)(lnormFactors[i]);
		...
		for (i=0;i<numVerts;i++)
		{
			 ...
			 || (i == (numVerts-1)))
			{
				if (vertcount == 1)
				{
					...
					for (m=0;m<3;m++)
					{
						v = (float)floor(((verts[i].pos.v[m] - translates[verts[i].groupNum].v[m]) / scales[verts[i].groupNum].v[m]) + 0.5f);
						if (v > 255) v = 255;
						if (v < 0) v = 0;
						vbyte = (U8)v;
						VCR_WriteByte(vbyte);
				...
				if (vertcount > 1)
				{
					...
					for (k=startvert;k<(startvert+vertcount);k++)
					{
						...
						for (m=0;m<3;m++)
						{
							v = (float)floor(((verts[k].pos.v[m] - translates[verts[k].groupNum].v[m]) / scales[verts[k].groupNum].v[m]) + 0.5f);
							if (v > 255) v = 255;
							if (v < 0) v = 0;
							vbyte = (U8)v;
							VCR_WriteByte(vbyte);
...
void modelSequence_t::Play(void)
{
	if (playing)
		return;
	playStartTime = mesh_app.get_frame_begin();
	playing = 1;
}
...
U32 modelMount_t::RecursiveCircularCheck(int mIndex)
{
	modelMount_t *m;
	meshTri_t *mTri;
...
int modelMount_t::UpdateTransform(float updateTime, U32 forceUpdate)
{
	plane_t pln;
	vector_t pv[3];
	matrix_t rotTemp;
...
void modelMount_t::MountToWorld(vector_t &point)
{
	RecursiveMountToWorld(point, mesh_app.get_frame_begin());
}

void modelMount_t::WorldToMount(vector_t &point)
{
	RecursiveWorldToMount(point, mesh_app.get_frame_begin());
}
...
int model_t::ImportFrames(CC8 *filename, U32 forceRestart)
{
	...
	for (i=0;i<numframes;i++)
	{
		...
		if (preserveBaseframe)
		{
			...
			preserveVerts=null;
			preserveBaseTris=null;
			preserveMeshTris=null;
			preserveBaseframe = 0;
		}
	}
	if (frameVerts)
...
int model_t::SaveMDX(CC8 *filename)
{
	mdxChunkFile_t cf;
	byte *buffer;
	...
	MRL_ITERATENEXT(f,f,frames) // mdxmountframe_t mountFrames[numFrames]
	{
		...
		for (i=0;i<WS_MAXMOUNTS;i++)
		{
			...
			else
			{
				...
				mounts[i].UpdateTransform(mesh_app.get_frame_begin(), true);
	...
	if (fverts)
	{
		for (i=0;i<refFrame->numVerts;i++)
		{
			...
			for (m=0;m<3;m++)
			{
				v = (float)floor(((fverts[i].pos.v[m] - translates[fverts[i].groupNum].v[m]) / scales[fverts[i].groupNum].v[m]) + 0.5f);
				if (v > 255) v = 255;
				if (v < 0) v = 0;
				vbyte = (U8)v;
				VCR_WriteByte(vbyte);
			}
			lnorms[i] /= (float)(lnormFactors[i]);
			lnorms[i].Normalize();
			for (m=0;m<3;m++)
			{
				vbyte = (U8)(fabs(lnorms[i].v[m]*127));
				if (lnorms[i].v[m] < 0)
					vbyte |= 128;
				VCR_WriteByte(vbyte);
			}
	...
	MRL_ITERATENEXT(s,s,seqs)
	{
		...
		{
			if (trigger->trigger)
				blen += fstrlen(trigger->trigger)+1;
		...
		for (trigger=s->triggers.next;trigger!=&s->triggers;trigger=trigger->next)
		{
			...
			if (trigger->trigger || trigger->triggerBinData)
			{
				VCR_WriteInt(ptotal);
				if (trigger->trigger)
					ptotal += fstrlen(trigger->trigger)+1;
				if (trigger->triggerBinData)
...
{
	/* make sure we delete old name first in case he's not destructing objects again*/
	if (skin->path)
		delete skin->path;

	CC8 *ext=fget_extension(filename);

	U32 len;

	len=fstrlen(filename);
	len+=fstrlen(pathname);
	/* cushion fo extension etc */
	len+=16;

	autochar  path;
	path=(char *)xmalloc(len);

	char *end=fstrcpy(path,pathname);
	fstrcpy(end,filename);
	
	/* see if filename conflict */
	if (!ext)
	{
		U32 has_bmp,has_tga;
		...
		/* sending end as beginning since */
		/* extension is definately after */

		fset_extension(end,"bmp");
		has_bmp=file_exist(path);
		
		fset_extension(end,"tga");
		has_tga=file_exist(path);
		...
		if (has_bmp && has_tga)
		{
			...
			{
				autochar allocd_str = (char *)SYS_OpenFileBox("Image Files (*.bmp,*.tga)\0" "*.bmp;*.tga\0" "24-bit Bitmap Files (*.bmp)\0" "*.bmp\0" "32-bit Targa Files (*.tga)\0" "*.tga\0\0", "Open Skin",null);
				if (!allocd_str)
					return FALSE;

				U32 new_len=fstrlen(allocd_str);
				if ((new_len+1) > len)
				{
					xfree(path);
					path=CStr(allocd_str);
				}
				else
					fstrcpy(path,allocd_str);
			}
			else
				return FALSE;
		}
		else if (has_bmp) /* rest to tga if we are a bmp */
		{
			fset_extension(end,"bmp");
		}

		ext=fget_extension(path);
	}
	...
	fstrcpy(skin->name,fget_filename(path));
	skin->path=(char *)path.release();
	skin->tex=vid->TexLoad(skin->path,false,TEX_LOAD_MASKED,0x00FF00FF);
	if (!skin->tex)
		return FALSE;
...
int model_t::LoadMDX(CC8 *filename, U32 useProjectData)
{
	mdxChunkFile_t cf;
	mdxskin_t tSkin;
	modelFrame_t *f;
		...
		if (version == 2)
		{
			...
			for (i=0;i<count;i++)
			{
				...
				if (s->framesPerSecond < 0.0)
					s->framesPerSecond = 1000.0f / VCR_ReadInt();
				else
					VCR_ReadInt();
				tpos = VCR_ReadInt();
				...
				if (tpos)
				{
					...
					item->trigger = ALLOC(char, fstrlen((char *)fseqChunk->data + tpos)+1);
		...
		else if (version == 3)
		{
			...
			for (i=0;i<count;i++)
			{
				...
				if (tpos)
				{
					...
					trigger->trigger = ALLOC(char, fstrlen((char *)fseqChunk->data + tpos)+1);
